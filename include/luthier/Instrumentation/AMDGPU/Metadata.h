...
struct Metadata final {
  ...
  std::optional<AccessQualifier> ActualAccQual{
      std::nullopt};
...
class AMDGCNMetadataParserAnalysisPass
    : public llvm::AnalysisInfoMixin<AMDGCNMetadataParserAnalysisPass> {
  friend AnalysisInfoMixin;

  static llvm::AnalysisKey Key;

  const amdgpu::hsamd::MetadataParser &Parser;

public:
  class Result {
    friend AMDGCNMetadataParserAnalysisPass;

    /// The metadata object
    std::unique_ptr<amdgpu::hsamd::Metadata> ObjMetadata;
    /// The metadata of the kernel being lifted
    std::unique_ptr<amdgpu::hsamd::Kernel::Metadata> KernelMetadata;

    Result(std::unique_ptr<amdgpu::hsamd::Metadata> ObjMetadata,
           std::unique_ptr<amdgpu::hsamd::Kernel::Metadata> KernelMetadata)
        : ObjMetadata(std::move(ObjMetadata)),
          KernelMetadata(std::move(KernelMetadata)) {};

  public:
    const amdgpu::hsamd::Metadata &getObjectMetadata() const {
      return *ObjMetadata;
    }
  ...
  explicit AMDGCNMetadataParserAnalysisPass(
      const amdgpu::hsamd::MetadataParser &Parser)
      : Parser(Parser) {};
