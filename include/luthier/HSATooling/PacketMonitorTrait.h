//===-- PacketMonitorTrait.h - the tool's view of every AQL packet -*- C++ -*-===//
...
///
/// \file
/// Header-only CRTP trait that delivers the application's AQL packets to a tool,
/// through whichever of two doors the application left open.
///
...
/// INVALID (at \c amd_aql_queue.cpp:122). That is an HSA promise, not a driver or
...
/// \c ioctl and the \c open family are wrapped with \c audit_hooks::register_wrap
/// from the trait's constructor. The tool \e is the audit_hook plugin: it is
/// loaded into the auditor's linker namespace out of \c AH_PLUGINS, so by the
/// time this trait is constructed the \c audit_hooks API is available and the
/// registration is in time for the application's first call.
///
/// This replaced GOTCHA, for a mechanical reason rather than a stylistic one.
/// GOTCHA rewrites GOT entries in place, which means flipping page protections on
/// live memory; in a process that also hosts rocprofiler-sdk's bundled libgotcha
/// that is two independent rewriters over the same slots, ordered only by
/// whichever constructor ran first. Under LD_AUDIT the dynamic linker keeps
/// ownership of the GOT and hands the auditor the original function pointer at
/// bind time, so there is nothing to race over and nothing to resolve by search:
...
/// counted recursion depth and killed the process at 32 with a message, because a
/// preloaded library that \e defines \c ioctl and chains with
...
/// (\c tls_hooks_paused). The guard was for a mechanism that is no longer in use.
///
/// \par Nothing here is shared between tools
/// Every field is an instance field. The KFD path used to be file-scope state in
/// \c QueueWrapper.cpp -- one queue table, one callback chain, one poller thread,
/// one set of counters per process -- which meant two tools in one process
/// silently shared a tracking table and a 64-queue budget. The only statics left
/// are the ones the mechanism forces, and each is a member of
/// \c PacketMonitorTrait<Derived> rather than of a common base, so even those are
/// per-tool rather than per-process:
/// \li the \c Real* function pointers, whose addresses are handed to \c ld.so at
...
///     busy queue this is one line per packet, and a runaway once produced 18.6
...
private:
  ...
  static constexpr unsigned PacketTypeKernelDispatch = HSA_PACKET_TYPE_KERNEL_DISPATCH;
  ...
  /// which is what stops the GPU seeing a header for a packet whose body has not
  /// landed.
  ...
  /// reads entries without the lock, so "nobody will ever touch this again" is a
  ...
  struct ForwardedQueue {
    ...
    /// them differently: the GPU tracks its progress against the application's
    /// own write pointer, which we do not intercept. Emitting a different count
    /// means taking that over too.
    uint64_t Produced;
    ...
    /// wrapped the ring while we were behind, so packets were overwritten before
    /// we copied them -- see \c OverrunPackets.
    uint64_t MaxLag;
    ...
    /// it cannot free a slot without the GPU consuming our copy of it. Kept as a
...
public:
  ...
  ///
  /// Installs the \c hsa_queue_create wrapper through the rocprofiler API tables
  /// and registers the driver-boundary hooks with the audit engine. Neither is
  /// conditional on the other: which door a given application actually delivers
  /// packets through is a property of that application, not of how the tool was
  /// constructed, and an application can use both -- one that drives KFD itself
  /// and then has HSA brought up inside it does exactly that.
  ///
  /// \param Err receives an error when either installation is refused. An audit
  /// registration that fails is a real failure rather than a quiet no-op: a tool
  /// that attaches and then observes nothing looks exactly like an application
  /// that dispatched nothing, which is the failure mode this whole path is most
  /// prone to.
  ///
  ...
  PacketMonitorTrait(
    ...
    Verbose = verboseFromEnvironment();
  ...
  ///
  /// The HSA door needs no teardown -- its wrappers are never uninstalled -- but
  /// the KFD door owns a thread that reads this object's fields, so it has to be
  /// stopped before they go away. That is new with per-instance state: while the
  /// queue table was file-scope the poller could simply run until the process
  /// exited.
  ~PacketMonitorTrait() {
    stopPoller();
    summarizeAll();
  ...
  /// own callback. ROCr guards the equivalent case in its own interception layer.
  ///
  ...
  /// permits one such VM per GPU per process, and \c hsa_init inside that process
  /// therefore fails -- measured both orders: the application's \c ACQUIRE_VM then
  /// \c hsa_init gives \c HSA_STATUS_ERROR_OUT_OF_RESOURCES, and the reverse makes
  /// the application's \c ACQUIRE_VM fail with \c EBUSY.
  ///
  ...
  /// We see only an \c ioctl on a descriptor; nothing in that call says who asked
  /// for it. So the tool has to say so. A thread-local flag is enough because of
  /// a measured fact rather than an assumption: every queue the runtime created
  /// in response to \c hsa_queue_create appeared on the **calling thread**
  ...
  /// appear on the calling thread. It is \b not enough for \c hsa_init: bringing
  /// the runtime up spawns its own threads and creates queues on them, and
  /// measured in a KFD application the runtime's queue was wrapped as the
  ...
  /// leave the thread permanently marked as the tool, and from then on none of
  /// the application's queues would be instrumented -- a silent, total loss of
  /// interception, which is the worst failure this path has.
  class ToolRegion {
  public:
  ...
  public:
    explicit ProcessWideToolRegion(PacketMonitorTrait &Monitor)
        : Monitor(Monitor) {
  ...
  /// when packets flow through it -- and the case that matters most is one where
  /// they may not. When a tool initialises HSA, the runtime creates AQL queues on
  /// the tool's behalf; we must leave those alone, but they can sit empty, so no
  /// callback would ever fire either way. A count separates "correctly ignored"
  ...
  ///
  /// More than one component may want a turn at each packet -- Luthier alongside
  /// a profiler, say -- and the HSA runtime supports exactly that, so a
  /// driver-level replacement that did not would be a step backwards.
  ///
  /// \par Order
  ...
  /// by having a callback pass packets to a writer, and not calling it drops
  /// them. That is deferred: emitting a different number of packets than the
  /// application submitted invalidates the index-sequence check, which is
  ...
  CallbackHandle addPacketCallback(PacketCallback CB,
                                        void *UserData) {
    if (CB == nullptr)
    ...
    if (H == InvalidCallbackHandle &&
        CallbackCount < MaxPacketCallbacks) {
  ...
  void removePacketCallback(CallbackHandle H) {
    if (H < 0 || static_cast<unsigned>(H) >= MaxPacketCallbacks)
      return;
    pthread_mutex_lock(&QueueLock);
    __atomic_store_n(&Callbacks[H].CB,
                     static_cast<PacketCallback>(nullptr),
                     __ATOMIC_RELEASE);
  ...
  void setPacketCallback(PacketCallback CB, void *UserData) {
    pthread_mutex_lock(&QueueLock);
    for (unsigned I = 0; I < CallbackCount; I++) {
      __atomic_store_n(&Callbacks[I].CB,
                       static_cast<PacketCallback>(nullptr),
                       __ATOMIC_RELEASE);
  ...
  ///
  /// A static function over a plain array, rather than something that reads this
  /// object, so the ordering guarantee above can be tested without a GPU, a queue
  /// or a driver -- which is where a claim like "last registered runs first"
  /// belongs, since it is otherwise the kind of thing that is asserted in a
  /// comment and never checked.
  ///
  /// Entries whose \c CB is null are skipped, which is how removal works without
  /// shuffling the others.
  static void runCallbackChain(const CallbackEntry *Entries,
                               unsigned Count, const QueueInfo &Q,
                               uint64_t PacketIndex, hsa::AqlPacket &Packet) {
  ...
  void runRegisteredCallbacks(const QueueInfo &Q, uint64_t PacketIndex,
                              hsa::AqlPacket &Packet) {
    runCallbackChain(Callbacks,
                     __atomic_load_n(&CallbackCount, __ATOMIC_ACQUIRE), Q,
                     PacketIndex, Packet);
  }

  //===--------------------------------------------------------------------===//
  // Which GPU an address belongs to
  //===--------------------------------------------------------------------===//
  ...
  /// \c gpu_id, and the allocation map recorded it when the application made the
  /// call. So the descriptor's own address answers the question.
  ///
  ...
  /// survives the tool handing work to another thread; it answers for any address,
  /// not only while a callback is on the stack; and it cannot silently report the
  /// wrong device for a kernel that was allocated on one GPU and dispatched from
  /// a queue on another.
  ///
  /// \return the owning \c gpu_id, or an error naming \p Addr when no tracked
  /// allocation contains it -- which means either that the allocation predates
  /// this tool attaching, or that the address is not device memory at all.
  llvm::Expected<uint32_t> gpuIdForAddress(uint64_t Addr) const {
    if (auto A = kfd::findAllocation(Addr))
      return A->GpuId;
  ...
  int handleIoctl(int Fd, unsigned long Request, void *Arg) {
    ...
    // Remember which DRM descriptor each GPU's memory is bound to. Observed
    // rather than opened, because an mmap offset only resolves on the descriptor
    // that created the allocation -- see recordGpuDrmFd. Recorded even inside a
    // tool region: the descriptor is a property of the GPU, not of who asked.
    if (IsKfd && Nr == AcquireVmNr && Arg != nullptr) {
      ...
      if (Ret == 0) {
        kfd::recordGpuDrmFd(V->gpu_id, static_cast<int>(V->drm_fd));
        if (Verbose)
          fprintf(stderr, "%sacquire_vm gpu=%u drm_fd=%d\n", LogPrefix,
                  V->gpu_id, static_cast<int>(V->drm_fd));
      }
      return Ret;
    }
    ...
    // the caller is merely late, which is exactly the situation.
    ...
    // hsaKmtGetRuntimeCapabilities instead, so nothing depends on this one being
    // populated.
    //
  ...
  /// by design. HSA cannot live there. It talks to the driver through descriptors
  /// the application opened and through a DRM address space the application
  /// bound, and a second copy of the runtime in a second namespace would be a
  /// second party contending for per-process resources rather than a passenger on
  /// the application's. So it is loaded with \c dlmopen into \c LM_ID_BASE: the
  /// application's namespace, where it sees the application's descriptors.
  ///
  ...
  /// rather than from a constructor. The ordering is forced: the application must
  /// claim the driver's per-process resources first. Initializing HSA earlier does
  ...
  /// theirs is a crash in someone else's program. Hooking the \e open is what makes
  /// "the application has started using the driver" observable at the earliest
  /// point where it is also already true.
  ///
  /// \par The three collisions this works around
  /// Each needed a different mechanism, which is worth knowing before assuming a
  /// fourth would yield to the same trick:
  ...
  ///     refuses a second \e space rather than a second \e call (\c FdSharing.h);
  ...
  llvm::Error ensureHsaInitializedInApplication() {
    ...
    llvm::Error Err = llvm::Error::success();
    {
      ...
      // runtime's own queues as the application's and feed our dispatches to our
      // own callback.
      //
      // Process-wide, not the per-thread region: bringing the runtime up creates
      // queues on threads it spawns itself, so a thread-local flag does not cover
      // them. Measured -- with the per-thread region the runtime's queue was
      // wrapped as the process's second queue.
      ...
      AppHsaHandle = dlmopen(LM_ID_BASE, "libhsa-runtime64.so",
        ...
        if (HsaInit == nullptr) {
          ...
          if (St != HSA_STATUS_SUCCESS)
            Err = LUTHIER_MAKE_GENERIC_ERROR(llvm::formatv(
                ...
                "drives the KFD driver. Run with LUTHIER_VERBOSE=1: every "
  ...
  /// \brief The handle for the HSA runtime living in the application's namespace,
  /// or \c nullptr before \c ensureHsaInitializedInApplication has run.
...
private:
  ...
  /// porting this anywhere with a different convention, or before adding a filter
  /// that reads \c Arg earlier than the descriptor check.
  ...
  /// reason it is safe for \c ioctl, and the mode is only ever read when the flags
  /// say it is there.
  ...
  ///
  /// Static rather than an instance field, and this is the mechanism forcing it:
  /// the address of each is handed to the audit engine at registration, and
  /// audit_hook has no unregister call, so the storage has to outlive any one
  /// instance. Being members of \c PacketMonitorTrait<Derived> they are still one
  /// set per tool rather than one per process.
  ...
  ///
  /// \c register_wrap rather than \c register_replace: other tools may be wrapping
  /// the same symbols, and wrapping chains where replacing discards. The engine
  /// writes the next link inward into \c Real*, which is why nothing here has to
  /// search for it.
  ...
  //
  // Reached through the singleton rather than through a captured pointer, for the
  // two reasons the HSA path does the same. Registration happens inside this
  // trait's constructor, so a call arriving before the tool finishes constructing
  // would otherwise reach a half-built Derived; and withInstance holds the
  // instance alive for the call, so a concurrent teardown cannot destroy the tool
  // underneath a hook that is already running. When there is no instance the call
  // is forwarded untouched, which is what makes attaching late safe.
  //===--------------------------------------------------------------------===//
  ...
  static int ioctlHook(int Fd, unsigned long Request, void *Arg) {
    ...
    const bool Handled =
        Singleton<Derived>::withInstance([&](Derived &Self) {
          Ret = static_cast<PacketMonitorTrait &>(Self).handleIoctl(Fd, Request,
                                                                    Arg);
        });
    if (!Handled)
      return RealIoctl(Fd, Request, Arg);
  ...
  /// Shared by all four open wrappers: hand back the bound descriptor if there is
  /// one, and take the application's first touch of the driver as the cue to bring
  /// HSA up in its namespace.
  ///
  ...
  /// missed tinygrad entirely, since CPython's \c os.open resolves to \c open64,
  /// and the symptom was the address-space collision looking exactly as though
  ...
  static int borrowedOrMinusOne(const char *Path) {
    ...
    (void)Singleton<Derived>::withInstance([&](Derived &Self) {
      ...
      //
      // Reported and then dropped rather than propagated: we are standing in for
      // the application's open(), and there is no way to return a Luthier error
      // through it. Failing the open instead would break an application that
      // never asked for any of this, over a runtime only the tool needs. The
      // message is what makes the subsequent "loader could not find an agent"
      // diagnosable.
  ...
  ///
  /// Aborting beats limping on when the engine never filled this in. A hook that
  /// intercepts an ioctl but cannot forward it leaves the application waiting for
  /// work the driver never received -- a hang with no error anywhere, which is
  ...
  /// number is cached, so the cost is one fstat per call and no more. An earlier
  /// design filtered on the request number first, which is free; that was
  /// dropped, and reinstating it is fine for the request (a scalar) but would be
  /// a real bug if the filter touched Arg, since nothing has yet established that
  ...
  void releaseRing(int Fd, uint64_t Handle, uint32_t GpuId, void *Pages,
    ...
    if (realIoctl(Fd, AMDKFD_IOC_FREE_MEMORY_OF_GPU, &FreeArgs) != 0 && Verbose)
  ...
  /// that a tool region opened inside another does not end interception when the
  /// inner one closes.
  ///
  ...
  void runCallback(ForwardedQueue &Q, uint64_t Index, hsa::AqlPacket &Packet) {
    if (Verbose && packetType(Packet.Packet.Header) == PacketTypeKernelDispatch) {
      ...
      memcpy(&KernelObject, Bytes + AqlKernelObjectOffset,
             sizeof(KernelObject));

      auto A = kfd::findAllocation(KernelObject);
      if (A)
        fprintf(stderr,
    ...
    (void)Singleton<Derived>::withInstance([&](Derived &Self) {
      Self.onDispatchPacket(Q.Info, Index, Packet);
    });
  }
  ...
  /// slot directly. Our slot's header is held at INVALID for the whole operation
  ...
  static void *pollerTrampoline(void *Self) {
    static_cast<PacketMonitorTrait *>(Self)->pollerMain();
    return nullptr;
  }
    ...
    while (!__atomic_load_n(&PollerStopRequested, __ATOMIC_ACQUIRE)) {
      ...
      for (int I = 0; I < N; I++) {
        ...
        while (Q.Consumed < Claimed && Budget > 0) {
          ...
          // window where the application could rewrite the slot cannot open at
          // all -- was tried as a fix for the S10b hang and changed nothing, so
  ...
  /// freed memory. While the table was file-scope the thread could simply outlive
  /// everything and be reaped by process exit.
  void stopPoller() {
    pthread_mutex_lock(&QueueLock);
  ...
  /// wait for two.
  ///
  /// Bounded, and deliberately not a guarantee. If the poller has stopped, or if
  /// a tool calls this from inside its own callback (which cannot make progress
  /// by definition), we give up after 5 ms rather than hang the application on
  /// its way out.
  ...
  /// interception permanently once it has created \c MaxTrackedQueues of them
  /// over its whole lifetime, even with only one alive at a time.
  ...
  static constexpr unsigned AllocMemoryNr =
      AMDKFD_IOC_ALLOC_MEMORY_OF_GPU & 0xFFu;
  ...
  int handleAllocMemory(int Fd, unsigned long Request, void *Arg) {
    ...
    const kfd::Allocation Recorded{A->va_addr, A->size,   A->flags,
                                   A->gpu_id,  A->handle, A->mmap_offset};
    kfd::recordAllocation(Recorded);
    // After recording, so an observer that immediately looks the address up finds
    // it.
    kfd::runAllocationCallbacks(Recorded);
  ...
  int handleFreeMemory(int Fd, unsigned long Request, void *Arg) {
    ...
    bool Removed = kfd::forgetAllocation(Handle);
    ...
    // did.
    kfd::runAllocationFreeCallbacks(Handle);
    if (Verbose)
  ...
  int handleCreateQueue(int Fd, unsigned long Request, void *Arg) {
    ...
    if (SlotIdx < 0) {
      ...
      // return -- it simply is not instrumented. stderr is easy to miss, so this
      // is a silent, permanent loss of interception for that queue rather than a
      // visible failure. If that ever needs to be detectable rather than merely
      // logged, expose the count and let the harness assert zero.
      fprintf(stderr,
  ...
  /// thread, and that thread has no business allocating.
  ...
  //=== Caches that used to be function-local statics ======================//
  bool Verbose{false};
  dev_t KfdRdev{0};
