...
/**
 * \brief If a tool contains a HIP device instrumentation function it \b must
 * use this macro once. Luthier HIP device instrumentation functions are marked
 * with the \p LUTHIER_DECLARE_FUNC and \p LUTHIER_EXPORT_FUNC macros.
 *
 * This macro defines a managed variable of type \p char named
 * \p __luthier_reserved in the code objects generated by HIP. This is so that
 * Luthier can:
 * 1. Identify a tool HSA code object by a constant time symbol lookup.
 * 2. Force the HIP runtime to load the instrumentation code objects right
 * before the first HIP kernel is launched
 * \sa LUTHIER_DECLARE_FUNC, LUTHIER_EXPORT_FUNC
 */
