...
/// remove packets -- see \c issue-85-ioctl-intercept-poc/EXPLORATION.md for why,
/// and what supporting that would require.
///
...
///     teardown either way.
/// \li \c LUTHIER_DEMO_ZERO_GRID=1 makes the built-in callback zero a dispatch's
///     grid, so no work-items launch. A self-test that the edit path is live.
//===----------------------------------------------------------------------===//
...
/// Luthier links the HSA runtime and may call \c hsa_init even when the
/// application under instrumentation never touches HSA. The runtime then creates
/// AQL queues for Luthier's own use, and those reach the driver through the same
/// \c ioctl this wrapper interposes. Wrapping them means instrumenting ourselves:
/// the tool's dispatches would be fed to the tool's own callback. ROCr guards the
/// equivalent case in its own interception layer.
///
