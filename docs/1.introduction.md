...
- __Instrumentation Hook__: A HIP ```__device__``` function written by the tool writer. It can be injected before or
  after instructions of a kernel or a device function. Hooks get inlined, and have an API allowing access to the GPU's
  ISA visible state.
...
trick to load before any of the ROCm libraries. When loaded, Luthier will initialize the HIP/HSA interception layers to
capture their API calls, and provide a callback to the tool writer and Luthier's internal components when requested.

Luthier uses the [rocprofiler-sdk](https://github.com/rocm/rocprofiler-sdk) API to get access to the HIP and HSA
API tables. Luthier then overwrites them with a version of each API that provides a callback when invoked.

Luthier supports selective API instrumentation; This means that if both the tool writer and Luthier's internal
components don't require a callback for an API, the original API will remain in place, and no additional overhead is
introduced.
...
user facing functionality if required. See [HSA Interceptor](3.hsa.md#hsa-interceptor),
[HIP Runtime API Interceptor](4.hip.md#hip-runtime-api-interceptor), and
[HIP Compiler API Interceptor](4.hip.md#hip-compiler-api-interceptor) for more details on how each API interceptor
works.
...
for AMD GPUs. See [The HSA Namespace](3.hsa.md#the-hsa-namespace) for more details on HSA and its part in Luthier. 
The reason we rely on HSA is the reason NVBit relies on the CUDA Driver API: it is the lowest common layer of programming
AMD GPUs for compute programs. This means that almost all compute runtimes rely primarily on HSA to load and launch
code on the device. To be as comprehensive as possible, and leverage the additional low-level info useful for 
instrumentation, Luthier uses HSA for instrumentation. 

## Intercepting Kernel Launches

Unlike the CUDA driver API, intercepting kernel launches require intercepting the exact moment 
To capture packets submitted to each queue, Luthier has to wrap each HSA queue created by the application with an
HSA intercept
On top of
the API tables, Luthier wraps each HSA queue created with an Interceptor Queue to get notified when a packet gets
submitted to the queue, which will be discussed
