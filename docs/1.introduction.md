...
- Insert (multiple) hooks inside the code between instructions, written with HIP as device functions.
- Remove/modify/add instructions to the original app.
...
to load and launch code on the device. To be as comprehensive as possible, and leverage the additional low-level info 
used for instrumentation, Luthier uses HSA for instrumentation. 
...
on to it and provides a callback right before the packet is about to get submitted.
