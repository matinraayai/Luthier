...
llvm::Expected<const CodeLifter::DisassemblyInfo &>
luthier::CodeLifter::getDisassemblyInfo(const luthier::hsa::ISA &isa) {
...
luthier::CodeLifter::disassemble(const hsa::ExecutableSymbol &symbol,
  ...
  if (!DisassembledSymbols.contains(symbol)) {
...
//      LUTHIER_RETURN_ON_ERROR(IsaOrError.takeError());
    ...
    auto codeOrError = symbol.getMachineCode();
    LUTHIER_RETURN_ON_ERROR(codeOrError.takeError());

    llvm::Expected<std::vector<llvm::MCInst>> instructionsOrError =
        disassemble(*isa, *codeOrError);
    LUTHIER_RETURN_ON_ERROR(instructionsOrError.takeError());

    DisassembledSymbols.insert(
        {symbol, std::make_unique<std::vector<hsa::Instr>>()});
    auto &out = DisassembledSymbols.at(symbol);
  ...
  return DisassembledSymbols.at(symbol).get();
}
...
luthier::CodeLifter::liftKernelModule(const hsa::ExecutableSymbol &Symbol,
  ...
  if (!KernelModules.contains(Symbol)) {
    ...
    auto targetFunctionInstsOrError =
        CodeLifter::instance().disassemble(Symbol);
      ...
      for (unsigned OpIndex = 0, E = mcInst.getNumOperands(); OpIndex < E;
        ...
        if (Op.isReg()) {
          ...
          if (MCID.isBranch()) {
            ...
            Builder.addSym(MCContext.createTempSymbol(".L123"),
