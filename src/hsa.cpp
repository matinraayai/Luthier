...
void getGpuAgents(llvm::SmallVectorImpl<GpuAgent> &agents) {
    ...
    auto returnGpuAgentsCallback = [](hsa_agent_t agent, void *data) {
        ...
        if (devType == HSA_DEVICE_TYPE_GPU) { agentMap->emplace_back(agent); }
        return stat;
    };
    LUTHIER_HSA_CHECK(coreTable.hsa_iterate_agents_fn(returnGpuAgentsCallback, &agents));
}
...
void getAllExecutables(llvm::SmallVectorImpl<Executable> &executables) {
    ...
    LUTHIER_HSA_CHECK(loaderApi.hsa_ven_amd_loader_iterate_executables(iterator, &executables));
}
