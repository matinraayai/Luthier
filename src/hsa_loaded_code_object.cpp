...
LoadedCodeObject::LoadedCodeObject(hsa_loaded_code_object_t lco) : HandleType<hsa_loaded_code_object_t>(lco),
                                                                   loaderApi_(luthier::HsaInterceptor::instance().getHsaVenAmdLoaderTable()) {}

Executable LoadedCodeObject::getExecutable() const {
    hsa_executable_t exec;
    LUTHIER_HSA_CHECK(loaderApi_.hsa_ven_amd_loader_loaded_code_object_get_info(
        this->asHsaType(),
...
luthier::co_manip::code_view_t LoadedCodeObject::getStorageMemory() const {
    ...
    LUTHIER_HSA_CHECK(loaderApi_.hsa_ven_amd_loader_loaded_code_object_get_info(this->asHsaType(),
                                                                                HSA_VEN_AMD_LOADER_LOADED_CODE_OBJECT_INFO_CODE_OBJECT_STORAGE_MEMORY_BASE,
                                                                                &storageBase));

    uint64_t storageSize;
    loaderApi_.hsa_ven_amd_loader_loaded_code_object_get_info(this->asHsaType(),
