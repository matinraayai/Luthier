    ...
    for (const auto &e: luthier::ContextManager::Instance().getHsaExecutables()) {
        for (const auto &a: luthier::ContextManager::Instance().getHsaAgents()) {
            for (const auto &s: e.getSymbols(a)) {
                if (s.getName().find(LUTHIER_RESERVED_MANAGED_VAR) != std::string::npos && s.getType() == HSA_SYMBOL_KIND_VARIABLE) {
...
const luthier::hsa::ExecutableSymbol &luthier::CodeObjectManager::getInstrumentationFunction(const void *wrapperKernelHostPtr,
    ...
    auto instrs = luthier::Disassembler::instance().disassemble(agent, f.getIndirectFunctionCode());
...
const luthier::hsa::ExecutableSymbol& luthier::CodeObjectManager::getInstrumentedKernel(const hsa::ExecutableSymbol& originalKernel) const {
    if (!instrumentedKernels_.contains(originalKernel))
        throw std::runtime_error(fmt::format("The Kernel Descriptor {:#x} has not been instrumented.", reinterpret_cast<luthier_address_t>(originalKernel.getKernelDescriptor())));
...
const luthier::hsa::ExecutableSymbol& luthier::CodeObjectManager::getInstrumentationKernel(const void *wrapperKernelHostPtr, hsa::GpuAgent agent) const {
