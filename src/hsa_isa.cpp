...
Isa Isa::fromName(const char *isaName) {
    ...
    LUTHIER_HSA_CHECK(coreApi.hsa_isa_from_name_fn(isaName, &isa));
    return Isa(isa);
}
std::string Isa::getName() const {
    uint32_t isaNameSize;
    LUTHIER_HSA_CHECK(
        getApiTable().core.hsa_isa_get_info_alt_fn(this->asHsaType(), HSA_ISA_INFO_NAME_LENGTH, &isaNameSize));
    std::string isaName(isaNameSize - 1, '\0');
    LUTHIER_HSA_CHECK(
        getApiTable().core.hsa_isa_get_info_alt_fn(this->asHsaType(), HSA_ISA_INFO_NAME, &isaName.front()));
    return isaName;
}

inline void parseIsaName(llvm::StringRef isaName, llvm::SmallVectorImpl<llvm::StringRef> &out) {
    isaName.split(out, '-', 4);
...
std::string Isa::getArchitecture() const {
    ...
    parseIsaName(isaName, isaNameComponents);
...
bool Isa::isXnacSupported() const {
    ...
    if (sRamECC == std::string::npos) return false;
    else { return isaName[sRamECC + strlen("xnack")] == '+'; }
}
bool Isa::isSRamECCSupported() const {
    std::string isaName = getName();
    auto sRamECC = isaName.find("sramecc");
    if (sRamECC == std::string::npos) return false;
    else { return isaName[sRamECC + strlen("sramecc")] == '+'; }
}
...
std::string Isa::getLLVMTargetTriple() const {
    ...
    return (llvm::Twine(isaNameComponents[0]) + "-" + llvm::Twine(isaNameComponents[1]) + "-"
            + llvm::Twine(isaNameComponents[2]) + "-" + llvm::Twine(isaNameComponents[3]))
        .str();
}
