 ...
 *
 *
 In CodeLifter: create one of this and map to the corresponding LCO (unordermap
 in the CodeLifter)
*/
...
private:
  /**
   * Represents the DWARF context of some ELF file.
   * It contains all the DWARF sections (debug_info, debug_line ...)
   */
  llvm::DWARFContext dwarfCtx;
   ...
   * (basically, a cache of the DIEs for each executable symbol in the LCO)
   */
  llvm::StringMap<std::optional<llvm::DWARFDie>> symbolNameToDie;

  // llvm::DenseMap<u_int64, llvm::DebugLoc>
  // llvm::DenseMap<>
  // llvm::DenseMap<>
   ...
   */
  void loadDebugInfo(const luthier::AMDGCNObjectFile &elfFile);
...
public:
   ...
   *
   */
  DWARFDebugInfo(const luthier::AMDGCNObjectFile &elfFile);
  ...
  getDIE(llvm::StringRef symbolName);
  ...
  getDebugLoc(llvm::StringRef symbolName, uint64_t instructionAddress,
              llvm::Module module, llvm::Metadata &instructionScope);
};
...
// 3) show how I create the DWARFDebugInfo and add it to the map in disassemble
// function! 4) should the map in this class be storing a unique pointer to the
// DIE? Or is this fine? Should I worry about the scope stuff here? (i think, as
// long as we a have a unique pointer to this object in the CodeLifter, then it
// won't be destructed prematurely, thus, the map won't be too)
// 5) should we still be caching the DebugLoc in the Instr?
// 6) VERY IMPORTANT:
//      initial goal: get some DebugLoc to be passed to the BuildMI(), which
//      currently invokes the default constructor now, we store DebugInfo for
//      each ExecutableSymbol for each LCO, but not the actual Instr -> so, how
//      do we use my work to extract accurate debug info for each Instr? MY BAD
//      FOR NOT THINKING THIS THROUGH PREVIOUSLY!
//
