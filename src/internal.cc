...
void editNoteData(elfio::Note &note);
...
extern "C" std::vector<hipModule_t> *__hipRegisterFatBinary(char *data) {
  ...
  char *header_buffer =
      new char[sizeof(*header) * sizeof(__ClangOffloadBundleHeader *)];
  std::memcpy(header_buffer, header,
              sizeof(*header) * sizeof(__ClangOffloadBundleHeader *));
  ...
  elfio::Note noteobj = getNote(&elfFile);
  editNoteData(noteobj);

  // OK, now I have to take this note section, put it back into the elf file,
  // and then get a new header from it...
  auto origNoteSec = elfFile.GetSectionByType("SHT_NOTE");

  // memcopy the modified note desc into the memory space of the original note
  // desc
  // Elf64_Shdr *new_note_hdr = reinterpret_cast<Elf64_Shdr *>(noteSec.Blob());
  // Elf64_Shdr *old_note_hdr = reinterpret_cast<Elf64_Shdr
  // *>(origNoteSec->Blob());

  // printf("%u | %u | %lu\n", new_note_hdr->sh_name, new_note_hdr->sh_type,
  // new_note_hdr->sh_addr); printf("%u | %u | %lu\n", old_note_hdr->sh_name,
  // old_note_hdr->sh_type, old_note_hdr->sh_addr);

  // std::memcpy(old_note_hdr, new_note_hdr, noteSec.TotalSize()); //seg faults

  // turns out, new_note_hdr and old_note_hdr have the same address. That's
  // probs why this seg faults

  // Right now, this doesn't do anything.
  modifiedHeader = reinterpret_cast<__ClangOffloadBundleHeader *>(
      reinterpret_cast<uintptr_t>(elfFile.Blob()) - desc->offset);

  // print instructions in elf .text section
  // To get the contents of the section, dump .sh_size bytes located at (char
  // *)p + shdr->sh_offset.
  ...
  // newWrapper->binary = reinterpret_cast<__ClangOffloadBundleHeader
  // *>(header_buffer);
