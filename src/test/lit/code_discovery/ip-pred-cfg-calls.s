...
//      IPPredicatedCFG must record an inter-procedural edge
//      from the calling MBB to _Z3foov's entry MBB.
...
// IPPredicatedCFG must record an inter-procedural edge from the kernel's direct-
// call MBB to _Z3foov's entry MBB, and mark the opaque-indirect-call MBB
// with "(has unresolved edges)". Individual MBB globalidx assignment can
// vary between runs (PrototypeCallGraph discovery order is DenseMap-
// keyed), so the checks below use CHECK-DAG and only pin down the CFG
// invariants that must hold regardless of enumeration order.

// A direct-call edge lands on _Z3foov's entry MBB.
// CHECK-DAG: Successors: [_Z3foov{{[^]]+}}]
// A return edge lands back on a _Z6kernelv MBB.
// CHECK-DAG: Predecessors: [_Z6kernelv{{[^]]*}}]
// _Z3foov's body contains the S_SETPC return.
// CHECK-DAG: S_SETPC_B64
