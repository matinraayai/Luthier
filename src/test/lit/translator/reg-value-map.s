...
// RUN:   '-passes=luthier-mock-load-amdgpu-code-objects,luthier-code-discovery,luthier-reg-value-map-printer' \
...
// Verifies RegValueMapAnalysis reconstructs per-block register-slice exit
// values purely from the luthier.reg / luthier.entry_reg_map metadata the
// translator emits.

// CHECK: RegValueMap for function 'vector_add': {{[1-9][0-9]*}} entry seeds
// CHECK: block %{{.*}}: {{[1-9][0-9]*}} register slices
// CHECK: reg {{[0-9]+}} +h{{[0-9]+}}:{{[0-9]+}} =
