...
//===----------------------------------------------------------------------===//
// Allocation observers, for S20
//===----------------------------------------------------------------------===//
std::mutex AllocObserverLock;
/// Which observer ran, in the order they ran. "HL" per allocation if the
/// high-priority one goes first, as it must.
std::string AllocOrder;
unsigned AllocSeenHigh = 0;
unsigned AllocSeenLow = 0;

using AddAllocCallbackFn = int (*)(void (*)(unsigned long long,
                                            unsigned long long, unsigned,
                                            unsigned, unsigned long long,
                                            unsigned long long, void *),
                                   void *, int);
using RemoveAllocCallbackFn = void (*)(int);
AddAllocCallbackFn AddAllocCallback = nullptr;
RemoveAllocCallbackFn RemoveAllocCallback = nullptr;

void onAllocHigh(unsigned long long, unsigned long long, unsigned, unsigned,
                 unsigned long long, unsigned long long, void *) {
  std::lock_guard<std::mutex> G(AllocObserverLock);
  AllocSeenHigh++;
  AllocOrder.push_back('H');
}

void onAllocLow(unsigned long long, unsigned long long, unsigned, unsigned,
                unsigned long long, unsigned long long, void *) {
  std::lock_guard<std::mutex> G(AllocObserverLock);
  AllocSeenLow++;
  AllocOrder.push_back('L');
}
...
using InterceptedCountFn = unsigned long long (*)();
AddCallbackFn AddCallback = nullptr;
RemoveCallbackFn RemoveCallback = nullptr;

/// This harness's own place in the packet chain.
int PacketHandle = -1;

/// How many ioctls the wrapper actually saw.
///
/// Resolving an exported symbol used to be sound evidence that interception was
/// live, because the symbol and the \c ioctl definition shipped in the same library
/// and a definition cannot fail. Under GOTCHA the symbols export whether or not
/// \c gotcha_wrap succeeded, and GOTCHA's library filters are process-global
/// mutable state a peer could narrow afterwards. So the guarantee has to be the
/// property -- traffic was seen -- rather than the proxy.
InterceptedCountFn InterceptedCount = nullptr;
...
bool connectToWrapper(bool Required) {
  ...
  // Presence is probed through luthierKfdSetPacketCallback but registration
  // does not use it. That call replaces the *whole* chain, which would evict any
  // tool already attached -- and the chain exists precisely so more than one
  // component can have a turn at each packet. A harness that took sole ownership
  // would make itself unusable as the application in any test where a Luthier
  // tool is also attached, which is exactly what kfd-lift-nonhsa-kernels does.
  ...
  InterceptedCount = reinterpret_cast<InterceptedCountFn>(
      dlsym(RTLD_DEFAULT, "luthierKfdInterceptedIoctlCount"));
  AddAllocCallback = reinterpret_cast<AddAllocCallbackFn>(
      dlsym(RTLD_DEFAULT, "luthierKfdAddAllocationCallback"));
  RemoveAllocCallback = reinterpret_cast<RemoveAllocCallbackFn>(
      dlsym(RTLD_DEFAULT, "luthierKfdRemoveAllocationCallback"));
  if (AddCallback == nullptr) {
    fprintf(stderr, "the wrapper is present but exports no "
                    "luthierKfdAddPacketCallback\n");
    return false;
  }
  PacketHandle = AddCallback(onPacket, nullptr);
  if (PacketHandle < 0) {
    fprintf(stderr, "the packet callback chain is full\n");
    return false;
  }
  WrapperPresent = true;
...
/// \return false if \p Required and the wrapper saw no ioctls at all.
///
/// Checked at teardown rather than at startup: at startup nothing has happened
/// yet, so a zero count would be correct and meaningless.
bool verifyInterceptionHappened(bool Required) {
  if (!Required || !WrapperPresent)
    return true;
  if (InterceptedCount == nullptr) {
    fprintf(stderr,
            "--require-wrapper was given, but this wrapper does not export "
            "luthierKfdInterceptedIoctlCount, so whether it intercepted "
            "anything cannot be established.\n");
    return false;
  }
  const unsigned long long Seen = InterceptedCount();
  if (Seen == 0) {
    fprintf(stderr,
            "--require-wrapper was given and the wrapper is loaded, but it "
            "intercepted 0 ioctls. It attached and then observed nothing, which "
            "would otherwise let every scenario below pass while measuring "
            "nothing.\n");
    return false;
  }
  printf("wrapper: intercepted %llu ioctls\n", Seen);
  return true;
}
...
int main(int Argc, char **Argv) {
  ...
  for (const auto &S : allScenarios()) {
    ...
    // Two components watching allocations.
    //
    // The registration order here is load-bearing, and getting it wrong once made
    // this scenario vacuous. The chain's tie-break is last-registered-first, so
    // registering LOW then HIGH makes *both* rules put HIGH first -- and the
    // scenario passed even with priority removed entirely. Registering HIGH first
    // is what makes the two rules disagree: priority still says "HL", while
    // ordering by registration alone would say "LH".
    int AllocHighHandle = -1, AllocLowHandle = -1;
    if (S.TwoAllocationObservers && WrapperPresent &&
        AddAllocCallback != nullptr) {
      std::lock_guard<std::mutex> G(AllocObserverLock);
      AllocOrder.clear();
      AllocSeenHigh = AllocSeenLow = 0;
      AllocHighHandle = AddAllocCallback(onAllocHigh, nullptr, /*Priority=*/100);
      AllocLowHandle = AddAllocCallback(onAllocLow, nullptr, /*Priority=*/1);
    }
    ...
    if (S.TwoAllocationObservers && WrapperPresent &&
        AddAllocCallback != nullptr) {
      std::lock_guard<std::mutex> G(AllocObserverLock);
      printf("    two allocation observers: high saw %u, low saw %u\n",
             AllocSeenHigh, AllocSeenLow);
      if (AllocSeenHigh == 0)
        V.fail("neither allocation observer saw anything, so this proves "
               "nothing about the chain. The wrapper allocates its own shim "
               "ring through these ioctls, so at least one allocation is "
               "expected for every wrapped queue");
      else if (AllocSeenHigh != AllocSeenLow)
        V.fail("the two allocation observers saw different counts (" +
               std::to_string(AllocSeenHigh) + " and " +
               std::to_string(AllocSeenLow) +
               ") -- the chain is not delivering to both");
      else {
        // The order must be HL for every allocation. Registration order was the
        // opposite, so "LH" here would mean priority is being ignored.
        bool OrderHeld = AllocOrder.size() % 2 == 0;
        for (size_t I = 0; OrderHeld && I + 1 < AllocOrder.size(); I += 2)
          OrderHeld = AllocOrder[I] == 'H' && AllocOrder[I + 1] == 'L';
        if (!OrderHeld)
          V.fail("the allocation observers did not run in priority order on "
                 "every allocation (saw \"" +
                 AllocOrder.substr(0, 16) +
                 "...\"); the high-priority observer was registered FIRST, so "
                 "\"LH\" here means the chain's last-registered-first tie-break "
                 "is deciding and priority is being ignored");
      }
      if (RemoveAllocCallback != nullptr) {
        RemoveAllocCallback(AllocHighHandle);
        RemoveAllocCallback(AllocLowHandle);
      }
    } else if (S.TwoAllocationObservers && WrapperPresent) {
      V.fail("this scenario needs luthierKfdAddAllocationCallback, which is not "
             "present -- it cannot check what it was asked to");
    }
  ...
  hsaKmtCloseKFD();
  if (!verifyInterceptionHappened(RequireWrapper))
    return 1;
