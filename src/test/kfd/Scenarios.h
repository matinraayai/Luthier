...
struct Scenario {
  ...
  /// Register two \e allocation observers, at different priorities, and require
  /// both to see every allocation in the priority order they asked for. Distinct
  /// from \c TwoCallbacks, which is about packets and whose chain deliberately has
  /// no priority at all.
  bool TwoAllocationObservers = false;
