...
std::vector<Scenario> allScenarios() {
  ...
  {
    Scenario X;
    X.Id = "S20-two-allocation-observers";
    X.Catches =
        "an allocation chain that delivers to one component but not the other, "
        "or that orders them by who registered rather than by the priority they "
        "asked for. Deterministic ordering is the only thing the GOTCHA "
        "mechanism buys over the callback array that preceded it, so if this is "
        "not checked on real driver traffic the justification for it is "
        "unverified. The ordering logic itself is pinned in the GPU-free unit "
        "tests; what this adds is that the chain is actually wired into the "
        "ioctl path";
    X.DispatchCount = 8;
    X.TwoAllocationObservers = true;
    S.push_back(X);
  }
  {
    Scenario X;
