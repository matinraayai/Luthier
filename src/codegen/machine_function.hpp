...
#ifndef CODEGEN_MACHINE_FUNCTION_HPP
#define CODEGEN_MACHINE_FUNCTION_HPP

#include "machine_basic_block.hpp"
#include "machine_instr.hpp"
#include "machine_mem_operand.hpp"
...
template<>
struct ilist_alloc_traits<luthier::MachineBasicBlock> {
    void deleteNode(luthier::MachineBasicBlock *MBB);
};

template<>
struct ilist_callback_traits<luthier::MachineBasicBlock> {
    void addNodeToList(luthier::MachineBasicBlock *N);
    void removeNodeFromList(luthier::MachineBasicBlock *N);
...
struct MachineFunctionInfo {
    ...
    virtual MachineFunctionInfo *clone(BumpPtrAllocator &Allocator, MachineFunction &DestMF,
...
class MachineFunctionProperties {
 ...
 public:
    ...
    bool hasProperty(Property P) const { return Properties[static_cast<unsigned>(P)]; }
    ...
    // in this.
    bool verifyRequiredProperties(const MachineFunctionProperties &V) const { return !V.Properties.test(Properties); }

    MachineFunctionProperties() : Properties(BitVector(static_cast<unsigned>(Property::LastProperty) + 1)) {
        llvm::outs() << "Properties constructor called!\n";
        llvm::outs() << "Size: " << Properties.size() << "\n";
    };
 ...
 public:
    ...
    class VariableDbgInfo {
     ...
     public:
        ...
        VariableDbgInfo(const DILocalVariable *Var, const DIExpression *Expr, MCRegister EntryValReg,
                        const DILocation *Loc)
            : Address(EntryValReg),
              Var(Var),
              Expr(Expr),
        ...
        bool inEntryValueRegister() const { return std::holds_alternative<MCRegister>(Address); }
        ...
        MCRegister getEntryValueRegister() const { return std::get<MCRegister>(Address); }
    ...
    class Delegate {
     ...
     public:
        ...
        /// directly.
        virtual void MF_HandleChangeDesc(MachineInstr &MI, const MCInstrDesc &TID) { return; }
    };
    ...
    struct ArgRegPair {
        Register Reg;
        uint16_t ArgNo;
        ArgRegPair(Register R, unsigned Arg) : Reg(R), ArgNo(Arg) { assert(Arg < (1 << 16) && "Arg out of range"); }
    };
 ...
 public:
    ...
    class DebugSubstitution {
     public:
        ...
        bool operator<(const DebugSubstitution &Other) const { return Src < Other.Src; }
    };
    ...
    MachineFunction(Function &F, const LLVMTargetMachine &Target, const TargetSubtargetInfo &STI, unsigned FunctionNum,
                    MachineModuleInfo &MMI);
    ...
    void resetDelegate(Delegate *delegate) {
        assert(TheDelegate == delegate && "Only the current delegate can perform reset!");
        TheDelegate = nullptr;
    }
    ...
    bool hasBBSections() const {
        return (BBSectionsType == BasicBlockSection::All || BBSectionsType == BasicBlockSection::List
                || BBSectionsType == BasicBlockSection::Preset);
    }

    /// Returns true if basic block labels are to be generated for this function.
    bool hasBBLabels() const { return BBSectionsType == BasicBlockSection::Labels; }
    ...
    MachineFunctionInfo *cloneInfoFrom(const MachineFunction &OrigMF,
                                       const DenseMap<MachineBasicBlock *, MachineBasicBlock *> &Src2DstMBB) {
        assert(!MFInfo && "new function already has MachineFunctionInfo");
        if (!OrigMF.MFInfo) return nullptr;
    ...
    /// for debugger use.
    /// \returns true if no problems were found.
    bool verify(LiveIntervals *LiveInts, SlotIndexes *Indexes, const char *Banner = nullptr,
    ...
    void insert(iterator MBBI, MachineBasicBlock *MBB) { BasicBlocks.insert(MBBI, MBB); }
    void splice(iterator InsertPt, iterator MBBI) { BasicBlocks.splice(InsertPt, BasicBlocks, MBBI); }
    void splice(iterator InsertPt, MachineBasicBlock *MBB) { BasicBlocks.splice(InsertPt, BasicBlocks, MBB); }
    ...
    unsigned getInstructionCount() const {
        ...
        for (const MachineBasicBlock &MBB: BasicBlocks) InstrCount += MBB.size();
        return InstrCount;
    }
    ...
    MachineInstr &cloneMachineInstrBundle(MachineBasicBlock &MBB, MachineBasicBlock::iterator InsertBefore,
    ...
    MachineMemOperand *getMachineMemOperand(MachinePointerInfo PtrInfo, MachineMemOperand::Flags f, uint64_t s,
                                            Align base_alignment, const AAMDNodes &AAInfo = AAMDNodes(),
                                            const MDNode *Ranges = nullptr, SyncScope::ID SSID = SyncScope::System,
                                            AtomicOrdering Ordering = AtomicOrdering::NotAtomic,
                                            AtomicOrdering FailureOrdering = AtomicOrdering::NotAtomic);

    MachineMemOperand *getMachineMemOperand(MachinePointerInfo PtrInfo, MachineMemOperand::Flags f, LLT MemTy,
                                            Align base_alignment, const AAMDNodes &AAInfo = AAMDNodes(),
                                            const MDNode *Ranges = nullptr, SyncScope::ID SSID = SyncScope::System,
                                            AtomicOrdering Ordering = AtomicOrdering::NotAtomic,
                                            AtomicOrdering FailureOrdering = AtomicOrdering::NotAtomic);
    ...
    MachineMemOperand *getMachineMemOperand(const MachineMemOperand *MMO, int64_t Offset, uint64_t Size) {
        return getMachineMemOperand(MMO, Offset, Size == ~UINT64_C(0) ? LLT() : LLT::scalar(8 * Size));
    }
    ...
    /// MachineMemOperands are owned by the MachineFunction and need not be
    /// explicitly deallocated.
    MachineMemOperand *getMachineMemOperand(const MachineMemOperand *MMO, const MachinePointerInfo &PtrInfo,
                                            uint64_t Size);
    ...
    MachineOperand *allocateOperandArray(OperandCapacity Cap) { return OperandRecycler.allocate(Cap, Allocator); }

    /// Dellocate an array of MachineOperands and recycle the memory. This is
    /// only intended for use by internal MachineInstr functions.
    /// Cap must be the same capacity that was used to allocate the array.
    void deallocateOperandArray(OperandCapacity Cap, MachineOperand *Array) { OperandRecycler.deallocate(Cap, Array); }
    ...
    /// the function.
    MachineInstr::ExtraInfo *createMIExtraInfo(ArrayRef<MachineMemOperand *> MMOs, MCSymbol *PreInstrSymbol = nullptr,
    ...
    /// comsumers.
    const std::vector<MCCFIInstruction> &getFrameInstructions() const { return FrameInstructions; }
    ...
    const std::vector<MCSymbol *> &getLongjmpTargets() const { return LongjmpTargets; }
    ...
    const std::vector<MCSymbol *> &getCatchretTargets() const { return CatchretTargets; }

    /// Add the specified symbol to the list of valid catchret targets for Windows
    /// EHCont Guard.
    void addCatchretTarget(MCSymbol *Target) { CatchretTargets.push_back(Target); }
    ...
    const std::vector<LandingPadInfo> &getLandingPads() const { return LandingPads; }
    ...
    void setWasmLandingPadIndex(const MachineBasicBlock *LPad, unsigned Index) { WasmLPadToIndexMap[LPad] = Index; }

    /// Returns true if the landing pad has an associate index in wasm EH.
    bool hasWasmLandingPadIndex(const MachineBasicBlock *LPad) const { return WasmLPadToIndexMap.count(LPad); }
    ...
    SmallVectorImpl<unsigned> &getCallSiteLandingPad(MCSymbol *Sym) {
        assert(hasCallSiteLandingPad(Sym) && "missing call site number for landing pad!");
        return LPadToCallSiteMap[Sym];
    }

    /// Return true if the landing pad Eh symbol has an associated call site.
    bool hasCallSiteLandingPad(MCSymbol *Sym) { return !LPadToCallSiteMap[Sym].empty(); }

    bool hasAnyCallSiteLabel() const { return !CallSiteMap.empty(); }

    /// Map the begin label for a call site.
    void setCallSiteBeginLabel(MCSymbol *BeginLabel, unsigned Site) { CallSiteMap[BeginLabel] = Site; }

    /// Get the call site number for a begin label.
    unsigned getCallSiteBeginLabel(MCSymbol *BeginLabel) const {
        assert(hasCallSiteBeginLabel(BeginLabel) && "Missing call site number for EH_LABEL!");
        return CallSiteMap.lookup(BeginLabel);
    }

    /// Return true if the begin label has a call site number associated with it.
    bool hasCallSiteBeginLabel(MCSymbol *BeginLabel) const { return CallSiteMap.count(BeginLabel); }

    /// Record annotations associated with a particular label.
    void addCodeViewAnnotation(MCSymbol *Label, MDNode *MD) { CodeViewAnnotations.push_back({Label, MD}); }

    ArrayRef<std::pair<MCSymbol *, MDNode *>> getCodeViewAnnotations() const { return CodeViewAnnotations; }

    /// Return a reference to the C++ typeinfo for the current function.
    const std::vector<const GlobalValue *> &getTypeInfos() const { return TypeInfos; }
    ...
    void setVariableDbgInfo(const DILocalVariable *Var, const DIExpression *Expr, MCRegister Reg,
                            const DILocation *Loc) {
        VariableDbgInfos.emplace_back(Var, Expr, Reg, Loc);
    }

    VariableDbgInfoMapTy &getVariableDbgInfo() { return VariableDbgInfos; }
    const VariableDbgInfoMapTy &getVariableDbgInfo() const { return VariableDbgInfos; }

    /// Returns the collection of variables for which we have debug info and that
    /// have been assigned a stack slot.
    auto getInStackSlotVariableDbgInfo() {
        return make_filter_range(getVariableDbgInfo(), [](auto &VarInfo) { return VarInfo.inStackSlot(); });
    }

    /// Returns the collection of variables for which we have debug info and that
    /// have been assigned a stack slot.
    auto getInStackSlotVariableDbgInfo() const {
        return make_filter_range(getVariableDbgInfo(), [](const auto &VarInfo) { return VarInfo.inStackSlot(); });
    }

    /// Returns the collection of variables for which we have debug info and that
    /// have been assigned an entry value register.
    auto getEntryValueVariableDbgInfo() const {
        return make_filter_range(getVariableDbgInfo(),
                                 [](const auto &VarInfo) { return VarInfo.inEntryValueRegister(); });
    }
    ...
    void addCallArgsForwardingRegs(const MachineInstr *CallI, CallSiteInfoImpl &&CallInfo) {
        ...
        bool Inserted = CallSitesInfo.try_emplace(CallI, std::move(CallInfo)).second;
        (void) Inserted;
...
template <> struct GraphTraits<luthier::MachineFunction*> :
    public GraphTraits<luthier::MachineBasicBlock*> {
    static NodeRef getEntryNode(luthier::MachineFunction *F) { return &F->front(); }

    // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
    using nodes_iterator = pointer_iterator<luthier::MachineFunction::iterator>;

    static nodes_iterator nodes_begin(luthier::MachineFunction *F) {
        return nodes_iterator(F->begin());
    }

    static nodes_iterator nodes_end(luthier::MachineFunction *F) {
        return nodes_iterator(F->end());
    }

    static unsigned       size       (luthier::MachineFunction *F) { return F->size(); }
};
template <> struct GraphTraits<const luthier::MachineFunction*> :
    public GraphTraits<const luthier::MachineBasicBlock*> {
    static NodeRef getEntryNode(const luthier::MachineFunction *F) { return &F->front(); }

    // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
    using nodes_iterator = pointer_iterator<luthier::MachineFunction::const_iterator>;

    static nodes_iterator nodes_begin(const luthier::MachineFunction *F) {
        return nodes_iterator(F->begin());
    }

    static nodes_iterator nodes_end  (const luthier::MachineFunction *F) {
        return nodes_iterator(F->end());
    }

    static unsigned       size       (const luthier::MachineFunction *F)  {
        return F->size();
    }
};
...
//
template <> struct GraphTraits<Inverse<luthier::MachineFunction*>> :
    public GraphTraits<Inverse<luthier::MachineBasicBlock*>> {
    static NodeRef getEntryNode(Inverse<luthier::MachineFunction *> G) {
        return &G.Graph->front();
    }
};
template <> struct GraphTraits<Inverse<const luthier::MachineFunction*>> :
    public GraphTraits<Inverse<const luthier::MachineBasicBlock*>> {
    static NodeRef getEntryNode(Inverse<const luthier::MachineFunction *> G) {
        return &G.Graph->front();
    }
};


void verifyMachineFunction(luthier::MachineFunctionAnalysisManager *,
                           const std::string &Banner,
                           const luthier::MachineFunction &MF);
