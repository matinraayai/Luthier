...
#ifndef CODEGEN_MACHINE_BASICBLOCK_H
#define CODEGEN_MACHINE_BASICBLOCK_H

#include "machine_instr.hpp"
#include "machine_instr_bundle_iterator.hpp"
...
struct MBBSectionID {
    ...
    bool operator==(const MBBSectionID &Other) const { return Type == Other.Type && Number == Other.Number; }
...
template<>
struct ilist_traits<luthier::MachineInstr> {
 private:
    friend class MachineBasicBlock;// Set by the owning MachineBasicBlock.

    luthier::MachineBasicBlock *Parent;

    using instr_iterator = simple_ilist<luthier::MachineInstr, ilist_sentinel_tracking<true>>::iterator;

 public:
    void addNodeToList(luthier::MachineInstr *N);
    void removeNodeFromList(luthier::MachineInstr *N);
    void transferNodesFromList(ilist_traits &FromList, instr_iterator First, instr_iterator Last);
    void deleteNode(luthier::MachineInstr *MI);
};
}

namespace luthier {

class MachineBasicBlock : public ilist_node_with_parent<MachineBasicBlock, MachineFunction> {
 public:
    ...
    struct RegisterMaskPair {
     public:
        ...
        RegisterMaskPair(MCPhysReg PhysReg, LaneBitmask LaneMask) : PhysReg(PhysReg), LaneMask(LaneMask) {}
 ...
 private:
    ...
    using const_probability_iterator = std::vector<BranchProbability>::const_iterator;
    ...
    std::optional<luthier::UniqueBBID> BBID;

    /// With basic block sections, this stores the Section ID of the basic block.
    luthier::MBBSectionID SectionID{0};
 ...
 public:
    ...
    bool hasAddressTaken() const { return MachineBlockAddressTaken || AddressTakenIRBlock; }
    ...
    using iterator = luthier::MachineInstrBundleIterator<MachineInstr>;
    using const_iterator = luthier::MachineInstrBundleIterator<const MachineInstr>;
    using reverse_iterator = luthier::MachineInstrBundleIterator<MachineInstr, true>;
    using const_reverse_iterator = luthier::MachineInstrBundleIterator<const MachineInstr, true>;
    ...
    const_instr_range instrs() const { return const_instr_range(instr_begin(), instr_end()); }

    iterator begin() { return instr_begin(); }
    const_iterator begin() const { return instr_begin(); }
    iterator end() { return instr_end(); }
    const_iterator end() const { return instr_end(); }
    reverse_iterator rbegin() { return reverse_iterator::getAtBundleBegin(instr_rbegin()); }
    const_reverse_iterator rbegin() const { return const_reverse_iterator::getAtBundleBegin(instr_rbegin()); }
    reverse_iterator rend() { return reverse_iterator(instr_rend()); }
    const_reverse_iterator rend() const { return const_reverse_iterator(instr_rend()); }

    /// Support for MachineInstr::getNextNode().
    static Instructions MachineBasicBlock::*getSublistAccess(MachineInstr *) { return &MachineBasicBlock::Insts; }

    inline iterator_range<iterator> terminators() { return make_range(getFirstTerminator(), end()); }
    inline iterator_range<const_iterator> terminators() const { return make_range(getFirstTerminator(), end()); }

    /// Returns a range that iterates over the phis in the basic block.
    inline iterator_range<iterator> phis() { return make_range(begin(), getFirstNonPHI()); }
    inline iterator_range<const_iterator> phis() const { return const_cast<MachineBasicBlock *>(this)->phis(); }
    ...
    const_pred_reverse_iterator pred_rbegin() const { return Predecessors.rbegin(); }
    ...
    const_succ_reverse_iterator succ_rbegin() const { return Successors.rbegin(); }
    ...
    inline iterator_range<pred_iterator> predecessors() { return make_range(pred_begin(), pred_end()); }
    inline iterator_range<const_pred_iterator> predecessors() const { return make_range(pred_begin(), pred_end()); }
    inline iterator_range<succ_iterator> successors() { return make_range(succ_begin(), succ_end()); }
    inline iterator_range<const_succ_iterator> successors() const { return make_range(succ_begin(), succ_end()); }
    ...
    void addLiveIn(const RegisterMaskPair &RegMaskPair) { LiveIns.push_back(RegMaskPair); }
    ...
    iterator_range<livein_iterator> liveins_dbg() const { return make_range(livein_begin_dbg(), livein_end()); }

    livein_iterator livein_begin() const;
    livein_iterator livein_end() const { return LiveIns.end(); }
    bool livein_empty() const { return LiveIns.empty(); }
    iterator_range<livein_iterator> liveins() const { return make_range(livein_begin(), livein_end()); }
    ...
    class liveout_iterator {
     public:
        ...
        liveout_iterator(const MachineBasicBlock &MBB, MCPhysReg ExceptionPointer, MCPhysReg ExceptionSelector,
                         bool End)
            : ExceptionPointer(ExceptionPointer),
              ExceptionSelector(ExceptionSelector),
              BlockI(MBB.succ_begin()),
              BlockEnd(MBB.succ_end()) {
            if (End) BlockI = BlockEnd;
            else if (BlockI != BlockEnd) {
                LiveRegI = (*BlockI)->livein_begin();
                if (!advanceToValidPosition()) return;
                if (LiveRegI->PhysReg == ExceptionPointer || LiveRegI->PhysReg == ExceptionSelector) ++(*this);
            }
        }
        ...
        liveout_iterator &operator++() {
            do {
                ++LiveRegI;
                if (!advanceToValidPosition()) return *this;
            } while ((*BlockI)->isEHPad()
                     && (LiveRegI->PhysReg == ExceptionPointer || LiveRegI->PhysReg == ExceptionSelector));
            return *this;
        }
        ...
        bool operator==(const liveout_iterator &RHS) const {
            if (BlockI != BlockEnd) return BlockI == RHS.BlockI && LiveRegI == RHS.LiveRegI;
            return RHS.BlockI == BlockEnd;
        }

        bool operator!=(const liveout_iterator &RHS) const { return !(*this == RHS); }

     private:
        bool advanceToValidPosition() {
            if (LiveRegI != (*BlockI)->livein_end()) return true;

            do { ++BlockI; } while (BlockI != BlockEnd && (*BlockI)->livein_empty());
            if (BlockI == BlockEnd) return false;
    ...
    liveout_iterator liveout_end() const { return liveout_iterator(*this, 0, 0, true); }
    iterator_range<liveout_iterator> liveouts() const { return make_range(liveout_begin(), liveout_end()); }
    ...
    void setMaxBytesForAlignment(unsigned MaxBytes) { MaxBytesForAlignment = MaxBytes; }
    ...
    unsigned getSectionIDNum() const {
        return ((unsigned) MBBSectionID::SectionType::Cold) - ((unsigned) SectionID.Type) + SectionID.Number;
    }
    ...
    bool isInlineAsmBrIndirectTarget() const { return IsInlineAsmBrIndirectTarget; }

    /// Indicates if this is the indirect dest of an INLINEASM_BR.
    void setIsInlineAsmBrIndirectTarget(bool V = true) { IsInlineAsmBrIndirectTarget = V; }
    ...
    bool sameSection(const MachineBasicBlock *MBB) const { return getSectionID() == MBB->getSectionID(); }
    ...
    void normalizeSuccProbs() { BranchProbability::normalizeProbabilities(Probs.begin(), Probs.end()); }
    ...
    MachineBasicBlock *getSingleSuccessor() {
        return const_cast<MachineBasicBlock *>(static_cast<const MachineBasicBlock *>(this)->getSingleSuccessor());
    }
    ...
    MachineBasicBlock *getSinglePredecessor() {
        return const_cast<MachineBasicBlock *>(static_cast<const MachineBasicBlock *>(this)->getSinglePredecessor());
    }
    ...
    const_iterator getFirstNonPHI() const { return const_cast<MachineBasicBlock *>(this)->getFirstNonPHI(); }
    ...
    const_iterator getFirstTerminator() const { return const_cast<MachineBasicBlock *>(this)->getFirstTerminator(); }
    ...
    const_iterator getFirstNonDebugInstr(bool SkipPseudoOp = true) const {
        return const_cast<MachineBasicBlock *>(this)->getFirstNonDebugInstr(SkipPseudoOp);
    }
    ...
    const_iterator getLastNonDebugInstr(bool SkipPseudoOp = true) const {
        return const_cast<MachineBasicBlock *>(this)->getLastNonDebugInstr(SkipPseudoOp);
    }
    ...
    /// return instruction.
    bool isEHScopeReturnBlock() const { return !empty() && back().isEHScopeReturn(); }
    ...
    void insert(iterator I, IT S, IT E) {
        assert((I == end() || I->getParent() == this) && "iterator points outside of basic block");
    ...
    instr_iterator insertAfterBundle(instr_iterator I, MachineInstr *MI) {
        assert((I == instr_end() || I->getParent() == this) && "iterator points outside of basic block");
        assert(!MI->isBundledWithPred() && !MI->isBundledWithSucc() && "Cannot insert instruction with bundle flags");
        while (I->isBundledWithSucc()) ++I;
    ...
    void splice(iterator Where, MachineBasicBlock *Other, iterator From) {
        ...
        if (Where != From) splice(Where, Other, From, std::next(From));
    }
    ...
    void splice(iterator Where, MachineBasicBlock *Other, iterator From, iterator To) {
        Insts.splice(Where.getInstrIterator(), Other->Insts, From.getInstrIterator(), To.getInstrIterator());
    }
    ...
    DebugLoc findDebugLoc(iterator MBBI) { return findDebugLoc(MBBI.getInstrIterator()); }
    ...
    DebugLoc rfindDebugLoc(reverse_iterator MBBI) { return rfindDebugLoc(MBBI.getInstrIterator()); }
    ...
    DebugLoc findPrevDebugLoc(iterator MBBI) { return findPrevDebugLoc(MBBI.getInstrIterator()); }
    ...
    DebugLoc rfindPrevDebugLoc(reverse_iterator MBBI) { return rfindPrevDebugLoc(MBBI.getInstrIterator()); }
    ...
    LivenessQueryResult computeRegisterLiveness(const TargetRegisterInfo *TRI, MCRegister Reg, const_iterator Before,
    ...
    std::optional<uint64_t> getIrrLoopHeaderWeight() const { return IrrLoopHeaderWeight; }
 ...
 private:
    ...
    const_probability_iterator getProbabilityIterator(const_succ_iterator I) const;
...
struct MBB2NumberFunctor {
    ...
    unsigned operator()(const MachineBasicBlock *MBB) const { return MBB->getNumber(); }
};
...
template <> struct GraphTraits<luthier::MachineBasicBlock *> {
    using NodeRef = luthier::MachineBasicBlock *;
    using ChildIteratorType = luthier::MachineBasicBlock::succ_iterator;

    static NodeRef getEntryNode(luthier::MachineBasicBlock *BB) { return BB; }
    static ChildIteratorType child_begin(NodeRef N) { return N->succ_begin(); }
    static ChildIteratorType child_end(NodeRef N) { return N->succ_end(); }
};

template <> struct GraphTraits<const luthier::MachineBasicBlock *> {
    using NodeRef = const luthier::MachineBasicBlock *;
    using ChildIteratorType = luthier::MachineBasicBlock::const_succ_iterator;

    static NodeRef getEntryNode(const luthier::MachineBasicBlock *BB) { return BB; }
    static ChildIteratorType child_begin(NodeRef N) { return N->succ_begin(); }
    static ChildIteratorType child_end(NodeRef N) { return N->succ_end(); }
};
...
//
template <> struct GraphTraits<Inverse<luthier::MachineBasicBlock*>> {
    using NodeRef = luthier::MachineBasicBlock *;
    using ChildIteratorType = luthier::MachineBasicBlock::pred_iterator;

    static NodeRef getEntryNode(Inverse<luthier::MachineBasicBlock *> G) {
        return G.Graph;
    }
...
template <> struct GraphTraits<Inverse<const luthier::MachineBasicBlock*>> {
    using NodeRef = const luthier::MachineBasicBlock *;
    using ChildIteratorType = luthier::MachineBasicBlock::const_pred_iterator;

    static NodeRef getEntryNode(Inverse<const luthier::MachineBasicBlock *> G) {
        return G.Graph;
    }
...
inline auto successors(const luthier::MachineBasicBlock *BB) { return BB->successors(); }
inline auto predecessors(const luthier::MachineBasicBlock *BB) {
...
class MachineInstrSpan {
    luthier::MachineBasicBlock &MBB;
    luthier::MachineBasicBlock::iterator I, B, E;

 public:
    MachineInstrSpan(luthier::MachineBasicBlock::iterator I, luthier::MachineBasicBlock *BB)
        : MBB(*BB), I(I), B(I == MBB.begin() ? MBB.end() : std::prev(I)),
          E(std::next(I)) {
        assert(I == BB->end() || I->getParent() == BB);
    }

    luthier::MachineBasicBlock::iterator begin() {
        return B == MBB.end() ? MBB.begin() : std::next(B);
    }
    luthier::MachineBasicBlock::iterator end() { return E; }
    bool empty() { return begin() == end(); }

    luthier::MachineBasicBlock::iterator getInitial() { return I; }
};
...
inline auto instructionsWithoutDebug(IterT It, IterT End,
                                     bool SkipPseudoOp = true) {
    return make_filter_range(make_range(It, End), [=](const luthier::MachineInstr &MI) {
