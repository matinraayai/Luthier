...
hipError_t hipArray3DCreate(hipArray * * array, const HIP_ARRAY3D_DESCRIPTOR * pAllocateArray) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray * *,const HIP_ARRAY3D_DESCRIPTOR *)>("hipArray3DCreate");
...
hipError_t hipArrayCreate(hipArray * * pHandle, const HIP_ARRAY_DESCRIPTOR * pAllocateArray) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray * *,const HIP_ARRAY_DESCRIPTOR *)>("hipArrayCreate");
...
hipError_t hipArrayDestroy(hipArray * array) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray *)>("hipArrayDestroy");
...
hipError_t hipFreeArray(hipArray * array) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray *)>("hipFreeArray");
			out = hipFunc(hipFuncArgs.array);
		};
		// Exit Callback
		if (isUserCallbackEnabled) hipUserCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_EXIT, apiId);
...
hipError_t hipMalloc3DArray(hipArray * * array, const struct hipChannelFormatDesc * desc, struct hipExtent extent, unsigned int flags) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray * *,const struct hipChannelFormatDesc *,struct hipExtent,unsigned int)>("hipMalloc3DArray");
...
hipError_t hipMallocArray(hipArray * * array, const hipChannelFormatDesc * desc, size_t width, size_t height, unsigned int flags) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray * *,const hipChannelFormatDesc *,size_t,size_t,unsigned int)>("hipMallocArray");
...
hipError_t hipMemcpy2DToArray(hipArray * dst, size_t wOffset, size_t hOffset, const void * src, size_t spitch, size_t width, size_t height, hipMemcpyKind kind) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray *,size_t,size_t,const void *,size_t,size_t,size_t,hipMemcpyKind)>("hipMemcpy2DToArray");
...
hipError_t hipMemcpy2DToArrayAsync(hipArray * dst, size_t wOffset, size_t hOffset, const void * src, size_t spitch, size_t width, size_t height, hipMemcpyKind kind, hipStream_t stream) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray *,size_t,size_t,const void *,size_t,size_t,size_t,hipMemcpyKind,hipStream_t)>("hipMemcpy2DToArrayAsync");
...
hipError_t hipMemcpyAtoH(void * dst, hipArray * srcArray, size_t srcOffset, size_t count) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(void *,hipArray *,size_t,size_t)>("hipMemcpyAtoH");
...
hipError_t hipMemcpyHtoA(hipArray * dstArray, size_t dstOffset, const void * srcHost, size_t count) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray *,size_t,const void *,size_t)>("hipMemcpyHtoA");
...
hipError_t hipMemcpyToArray(hipArray * dst, size_t wOffset, size_t hOffset, const void * src, size_t count, hipMemcpyKind kind) {
	...
	if (isUserCallbackEnabled || isInternalCallbackEnabled) {		auto& hipUserCallback = hipInterceptor.getUserCallback();
		...
		if (isInternalCallbackEnabled) hipInternalCallback(static_cast<void*>(&hipFuncArgs), LUTHIER_API_EVT_PHASE_ENTER, apiId, &skipFunction, &out);
		if (!skipFunction) {
			static auto hipFunc = hipInterceptor.getHipFunction<hipError_t(*)(hipArray *,size_t,size_t,const void *,size_t,hipMemcpyKind)>("hipMemcpyToArray");
