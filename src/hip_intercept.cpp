...
void __hipGetPCH(const char * * pch, unsigned int * size) {
	...
	if (enabledOp) hipUserCallback(static_cast<void*>(&hip_func_args), LUTHIER_API_EVT_PHASE_EXIT, api_id);
...
hipError_t hipCtxSynchronize() {
	...
	bool enabledOp = hipInterceptor.IsEnabledOps(api_id);
	std::optional<std::any> out{std::nullopt};
	if (enabledOp) hipUserCallback(nullptr, LUTHIER_API_EVT_PHASE_ENTER, api_id);
	...
	if (enabledOp) hipUserCallback(nullptr, LUTHIER_API_EVT_PHASE_EXIT, api_id);
