...
constexpr ElfSectionsDesc ElfSecDesc[] = {
    {LLVMIR, ".llvmir", 1, llvm::ELF::SHT_PROGBITS, 0,
    ...
    {CAL, ".text", 1, llvm::ELF::SHT_PROGBITS,
     llvm::ELF::SHF_ALLOC | llvm::ELF::SHF_EXECINSTR, "AMD CalImage"},
    {DLL, ".text", 1, llvm::ELF::SHT_PROGBITS,
     llvm::ELF::SHF_ALLOC | llvm::ELF::SHF_EXECINSTR, "x86 dll"},
    {STRTAB, ".strtab", 1, llvm::ELF::SHT_STRTAB, llvm::ELF::SHF_STRINGS,
     "String table"},
    {SYMTAB, ".symtab", sizeof(llvm::ELF::Elf64_Xword), llvm::ELF::SHT_SYMTAB,
     0, "Symbol table"},
    {RODATA, ".rodata", 1, llvm::ELF::SHT_PROGBITS, llvm::ELF::SHF_ALLOC,
     "Read-only data"},
    {SHSTRTAB, ".shstrtab", 1, llvm::ELF::SHT_STRTAB, llvm::ELF::SHF_STRINGS,
     "Section names"},
    ...
    {DEBUG_INFO, ".debug_info", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug info"},
    {DEBUG_ABBREV, ".debug_abbrev", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug abbrev"},
    {DEBUG_LINE, ".debug_line", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug line"},
    {DEBUG_PUBNAMES, ".debug_pubnames", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug pubnames"},
    {DEBUG_PUBTYPES, ".debug_pubtypes", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug pubtypes"},
    {DEBUG_LOC, ".debug_loc", 1, llvm::ELF::SHT_PROGBITS, 0, "Dwarf debug loc"},
    {DEBUG_ARANGES, ".debug_aranges", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug aranges"},
    {DEBUG_RANGES, ".debug_ranges", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug ranges"},
    {DEBUG_MACINFO, ".debug_macinfo", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug macinfo"},
    {DEBUG_STR, ".debug_str", 1, llvm::ELF::SHT_PROGBITS, 0, "Dwarf debug str"},
    {DEBUG_FRAME, ".debug_frame", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Dwarf debug frame"},
    {JITBINARY, ".text", 1, llvm::ELF::SHT_PROGBITS,
     llvm::ELF::SHF_ALLOC | llvm::ELF::SHF_EXECINSTR, "x86 JIT Binary"},
    {CODEGEN, ".cg", 1, llvm::ELF::SHT_PROGBITS, 0, "Target dependent IL"},
    {TEXT, ".text", 1, llvm::ELF::SHT_PROGBITS,
     llvm::ELF::SHF_ALLOC | llvm::ELF::SHF_EXECINSTR, "Device specific ISA"},
    {INTERNAL, ".internal", 1, llvm::ELF::SHT_PROGBITS, 0, "Internal usage"},
    {SPIR, ".spir", 1, llvm::ELF::SHT_PROGBITS, 0,
     "Vendor/Device-independent LLVM IR"},
    {SPIRV, ".spirv", 1, llvm::ELF::SHT_PROGBITS, 0, "SPIR-V Binary"},
    {RUNTIME_METADATA, ".AMDGPU.runtime_metadata", 1, llvm::ELF::SHT_PROGBITS,
     0, "AMDGPU runtime metadata"},
};
} // namespace
...
struct KernelParameterDescriptor {
  ...
  union InfoData {
    struct {
      ...
      uint32_t
          readOnly_ : 1; //!< OCL object is read only, applied to memory only
      ...
      uint32_t
          arrayIndex_ : 20; //!< Index in the objects array or LDS alignment
    };
    uint32_t allValues_;
  ...
  //    cl_kernel_arg_type_qualifier typeQualifier_; //!< Argument's type
  //    qualifier
...
struct WorkGroupInfo {
  ...
  KernelKind kind_{
      Normal}; //!< Kernel kind, is normal unless specified otherwise
  union Flags {
    struct {
...
getELFObjectFileBase(llvm::StringRef elf);
 ...
 */
llvm::Expected<std::unique_ptr<llvm::object::ELFObjectFileBase>>
getELFObjectFileBase(llvm::ArrayRef<uint8_t> elf);

llvm::Expected<llvm::msgpack::Document>
getElfNoteMetadataRoot(const llvm::object::ELFObjectFileBase *elf);

// class SymbolView;

///**
// * \briefs a non-owning read-only view of an AMDGPU ELF Code object located on
// the host
// */
...
//        if (io_ == std::nullopt) {
//            io_.emplace();
//            // All elfio objects are loaded with lazy=true in ElfViewImpl to
//            prevent additional memory copy if (not
//            io_->load(*dataStringStream_, true)) {
//            llvm::report_fatal_error("Failed to load the ELF file."); }
//        }
//        return io_.value();
//    }
//
...
//        initializeComgrMetaData(); return codeObjectVer_.value();
//    }
//
...
//    //    }
//
//    //    amd_comgr_metadata_node_t getKernelMetaDataMap(const std::string
//    &kernelSymbolName) const {
//    //        if (not kernelMetadataMap_.has_value())
//    initializeComgrMetaData();
//    //        auto kdInSymbolName = kernelSymbolName.find(".kd");
//    //        auto key = kdInSymbolName != std::string::npos ?
//    kernelSymbolName.substr(0, kdInSymbolName) : kernelSymbolName;
//    //        return kernelMetadataMap_.value().at(key);
//    //    }
//
//    //    WorkGroupInfo getAttrCodePropMetadata(amd_comgr_metadata_node_t
//    kernelMetaNode);
//
// private:
...
//    //    mutable std::optional<amd_comgr_data_t> comgrData_{std::nullopt};
//    //!< COMgr Data for the Executable ELF
//    //    mutable std::optional<amd_comgr_metadata_node_t>
//    metadata_{std::nullopt};//!< COMgr metadata mutable
//    std::optional<uint32_t> codeObjectVer_{std::nullopt};//!< version of code
//    object
//    //    mutable std::optional<amd_comgr_metadata_node_t>
//    kernelsMetadata_{std::nullopt};
//    //    mutable std::optional<std::unordered_map<std::string,
//    amd_comgr_metadata_node_t>> kernelMetadataMap_{std::nullopt}; const
//    byte_string_view data_; const std::unique_ptr<byte_char_stream_t>
//    dataStringStream_;//! Used to construct the elfio object;
//                                                                //! Without
//                                                                keeping a
//                                                                reference to
//                                                                this stream,
...
//
//     SymbolView(const std::shared_ptr<ElfView> &elf, const ELFIO::section
//     *section, std::string name,
//                luthier_address_t address, size_t size, size_t value, unsigned
//                char type)
//         : elf_(elf),
//           section_(section),
//           name_(std::move(name)),
...
//         return byte_string_view(reinterpret_cast<std::byte *>(address_),
//         size_);
//     }
//
...
//         return reinterpret_cast<const std::byte *>(section_->get_data() +
//         (size_t) value_
//                                                    - (size_t)
//                                                    section_->get_offset());
//     }
