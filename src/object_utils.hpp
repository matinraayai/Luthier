...
struct Metadata final {
    ...
    std::optional<decltype(llvm::AMDGPUAS::MAX_AMDGPU_ADDRESS)> AddressSpace = std::nullopt;
    ...
    std::optional<llvm::AMDGPU::HSAMD::AccessQualifier> ActualAccQual{std::nullopt};
 ...
 */
llvm::Expected<std::unique_ptr<llvm::object::ELF64LEObjectFile>> getAMDGCNObjectFile(llvm::StringRef Elf);
 ...
 * @return a \p std::unique_ptr pointing to a \p llvm::object::ELF64LEObjectFile
 */
llvm::Expected<std::unique_ptr<llvm::object::ELF64LEObjectFile>> getAMDGCNObjectFile(llvm::ArrayRef<uint8_t> Elf);

llvm::Expected<luthier::HSAMD::Metadata> parseMetaDoc(llvm::msgpack::Document &KernelMetaNode);

template<class ELFT>
static bool processNote(const typename ELFT::Note &Note, const std::string &NoteDescString,
                        llvm::msgpack::Document &Doc, llvm::msgpack::DocNode &Root) {

    if (Note.getName() == "AMD" && Note.getType() == llvm::ELF::NT_AMD_HSA_METADATA) {
        if (!Root.isEmpty()) { return false; }
        if (!Doc.fromYAML(NoteDescString)) { return false; }
        return true;
    }
    if (((Note.getName() == "AMD" || Note.getName() == "AMDGPU") && Note.getType() == llvm::ELF::NT_AMD_PAL_METADATA)
        || (Note.getName() == "AMDGPU" && Note.getType() == llvm::ELF::NT_AMDGPU_METADATA)) {
        if (!Doc.readFromBlob(NoteDescString, false)) { return false; }
...
llvm::Expected<luthier::HSAMD::Metadata> parseNoteMetaData(const llvm::object::ELFObjectFile<ELFT> *Obj) {
    bool Found = false;
    ...
    for (const auto &Shdr: *Sections) {
        ...
        for (const auto &Note: ELFFile.notes(Shdr, Err)) {
            DescString = Note.getDescAsStringRef(4);
            if (processNote<ELFT>(Note, DescString, Doc, Root)) { Found = true; }
        }
        LUTHIER_RETURN_ON_ERROR(Err);
    }

    if (Found) return parseMetaDoc(Doc);
    else
 ...
 * pointer of ELFSymbolRef type called getSymbolBy Name
 ...
 *
 * (Symbol LookUp through symtab and dynsymtab)
 * extract the symbol list from dynsymtab or symtab (https://github.com/ROCm/llvm-project/blob/79fe06ae87e47f983e358fdc804d07fb83c4d479/amd/comgr/src/comgr-symbol.cpp#L208-L291)
 * search for symbolname in dynsymtab first (bc it is shorter)
 * if not search for symbolname in symtab
...
llvm::raw_ostream &operator<<(llvm::raw_ostream &OS, const luthier::HSAMD::Metadata &MD);
