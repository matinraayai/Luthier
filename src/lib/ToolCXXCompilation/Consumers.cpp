...
/// Annotates \p FD with \p Marker if it is not already carrying that exact
/// annotation. The check is on \p FD alone, not its re-declarations.
void annotateWith(clang::ASTContext &Ctx, clang::FunctionDecl *FD,
                  llvm::StringRef Marker) {
  for (const auto *A : FD->specific_attrs<clang::AnnotateAttr>())
    if (A->getAnnotation() == Marker)
      return;
  FD->addAttr(clang::AnnotateAttr::Create(Ctx, Marker,
...
void annotateExportHandle(clang::ASTContext &Ctx, clang::FunctionDecl *FD,
                          bool AutoGenerated) {
  annotateWith(Ctx, FD,
               AutoGenerated ? SynthesizedExportFunctionHandleMarker
                             : ExportFunctionHandleMarker);
}
...
/// \c used, visibility, etc.), then forces \p Dst \c __host__. Also skips the
/// export-handle markers so they aren't accidentally propagated onto \p Dst
/// (the synthesis path re-annotates \p Dst explicitly).
  ...
  for (const clang::Attr *A : Src->attrs()) {
    ...
    if (const auto *Ann = llvm::dyn_cast<clang::AnnotateAttr>(A);
        Ann && (Ann->getAnnotation() == ExportFunctionHandleMarker ||
                Ann->getAnnotation() == SynthesizedExportFunctionHandleMarker))
      continue;
...
clang::FunctionDecl *makeHostHandle(clang::Sema &S, clang::FunctionDecl *Dev,
  ...
  annotateExportHandle(Ctx, Host, true);
  return Host;
}
...
bool EmitHostHandleForDevFuncConsumer::HandleTopLevelDecl(
  ...
  while (!Worklist.empty()) {
    ...
    if (Annotate.contains(Key)) {
      annotateExportHandle(Ctx, Dev, false);
      continue;
    }
