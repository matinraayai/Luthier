...
char LoadedCodeObjectSymbolImpl::ID = 0;

LoadedCodeObjectSymbolImpl::LoadedCodeObjectSymbolImpl(
    hsa::LoadedCodeObjectImpl LCO, llvm::object::ELFSymbolRef Symbol,
    std::optional<hsa::ExecutableSymbol> ExecutableSymbol)
    : llvm::RTTIExtends<LoadedCodeObjectSymbolImpl, LoadedCodeObjectSymbol>(),
      BackingLCO(std::move(LCO)),
      Symbol(std::make_unique<llvm::object::ELFSymbolRef>(Symbol)),
      ExecutableSymbol(std::move(ExecutableSymbol)) {};

llvm::Error LoadedCodeObjectSymbolImpl::fromExecutableSymbol(
    hsa_executable_symbol_t HsaSymbol) {
  llvm::Expected<luthier::address_t> LoadedAddressOrErr =
      hsa::ExecutableSymbolImpl(HsaSymbol).getAddress();
  LUTHIER_RETURN_ON_ERROR(LoadedAddressOrErr.takeError());
  return fromLoadedAddress(*LoadedAddressOrErr);
}

llvm::Error LoadedCodeObjectSymbolImpl::fromLoadedAddress(
  ...
  llvm::SmallVector<std::unique_ptr<LoadedCodeObject>, 4> LCOs;
  LUTHIER_RETURN_ON_ERROR(
      hsa::ExecutableImpl(Executable).getLoadedCodeObjects(LCOs));
...
llvm::Expected<hsa_agent_t> LoadedCodeObjectSymbolImpl::getAgent() const {
...
LoadedCodeObjectSymbolImpl::getExecutable() const {
...
LoadedCodeObjectSymbolImpl::getLoadedSymbolContents() const {
...
LoadedCodeObjectSymbolImpl::getLoadedSymbolAddress() const {
  ...
  auto SymbolLMO =
      getLoadedMemoryOffset(Symbol.getObject().getELFFile(), Symbol);
...
LoadedCodeObjectSymbolImpl::getExecutableSymbol() const {
  return ExecutableSymbol;
}

llvm::Expected<llvm::StringRef> LoadedCodeObjectSymbolImpl::getName() const {
  return Symbol.getName();
}

size_t LoadedCodeObjectSymbolImpl::getSize() const { return Symbol.getSize(); }

uint8_t LoadedCodeObjectSymbolImpl::getBinding() const {
  return Symbol.getBinding();
}

void LoadedCodeObjectSymbolImpl::print(llvm::raw_ostream &OS) const {
