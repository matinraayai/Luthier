...
///
/// \file
/// This file implements the \c hsa::LoadedCodeObjectImpl class, the concrete
/// implementation of the \c hsa::LoadedCodeObject interface.
//===----------------------------------------------------------------------===//
...
char LoadedCodeObjectImpl::ID = 0;

LoadedCodeObjectImpl::LoadedCodeObjectImpl(hsa_loaded_code_object_t LCO)
    : HandleType<hsa_loaded_code_object_t>(LCO) {}

llvm::Expected<std::unique_ptr<Executable>>
LoadedCodeObjectImpl::getExecutable() const {
...
LoadedCodeObjectImpl::getAgent() const {
  hsa_agent_t Agent;
  LUTHIER_RETURN_ON_ERROR(LUTHIER_HSA_SUCCESS_CHECK(
      getLoaderTable().hsa_ven_amd_loader_loaded_code_object_get_info(
          this->asHsaType(), HSA_VEN_AMD_LOADER_LOADED_CODE_OBJECT_INFO_AGENT,
          &Agent)));
  return std::make_unique<GpuAgentImpl>(Agent);
}

llvm::Expected<llvm::object::ELF64LEObjectFile &>
LoadedCodeObjectImpl::getStorageELF() const {
  auto &LCOCache =
...
llvm::Expected<long> LoadedCodeObjectImpl::getLoadDelta() const {
  long LoadDelta;
  LUTHIER_RETURN_ON_ERROR(LUTHIER_HSA_SUCCESS_CHECK(
      getLoaderTable().hsa_ven_amd_loader_loaded_code_object_get_info(
          this->asHsaType(),
...
LoadedCodeObjectImpl::getLoadedMemory() const {
...
llvm::Expected<std::string> LoadedCodeObjectImpl::getUri() const {
...
llvm::Expected<std::unique_ptr<ISA>> LoadedCodeObjectImpl::getISA() const {
  ...
  auto ISA = std::make_unique<hsa::ISAImpl>()->fromLLVM(
      std::get<0>(*ElfISAOrErr), std::get<1>(*ElfISAOrErr),
      std::get<2>(*ElfISAOrErr));
  return ISA;
}

llvm::Expected<const hsa::md::Metadata &>
LoadedCodeObjectImpl::getMetadata() const {
  auto &LCOCache =
      ExecutableBackedObjectsCache::instance().getLoadedCodeObjectCache();
  std::lock_guard Lock(LCOCache.ExecutableCacheMutex);
  LUTHIER_RETURN_ON_ERROR(LUTHIER_ERROR_CHECK(
...
llvm::Error LoadedCodeObjectImpl::getKernelSymbols(
...
llvm::Error LoadedCodeObjectImpl::getVariableSymbols(
    llvm::SmallVectorImpl<std::unique_ptr<hsa::LoadedCodeObjectSymbol>> &Out)
    const {
  llvm::Expected<luthier::AMDGCNObjectFile &> StorageElfOrErr = getStorageELF();
  LUTHIER_RETURN_ON_ERROR(StorageElfOrErr.takeError());
...
llvm::Error LoadedCodeObjectImpl::getDeviceFunctionSymbols(
    llvm::SmallVectorImpl<std::unique_ptr<hsa::LoadedCodeObjectSymbol>> &Out)
    const {
  llvm::Expected<luthier::AMDGCNObjectFile &> StorageElfOrErr = getStorageELF();
  LUTHIER_RETURN_ON_ERROR(StorageElfOrErr.takeError());
...
llvm::Error LoadedCodeObjectImpl::getExternalSymbols(
    llvm::SmallVectorImpl<std::unique_ptr<hsa::LoadedCodeObjectSymbol>> &Out)
    const {
  llvm::Expected<luthier::AMDGCNObjectFile &> StorageElfOrErr = getStorageELF();
  LUTHIER_RETURN_ON_ERROR(StorageElfOrErr.takeError());
...
llvm::Error LoadedCodeObjectImpl::getLoadedCodeObjectSymbols(
...
LoadedCodeObjectImpl::getLoadedCodeObjectSymbolByName(
...
llvm::Expected<llvm::ArrayRef<uint8_t>>
LoadedCodeObjectImpl::getStorageMemory() const {
