...
#define DEBUG_TYPE "luthier-hsa-agent"
...
agentForGpuId(const ApiTableContainer<::CoreApiTable> &CoreApi,
              uint32_t GpuId) {
  if (GpuId == 0)
  ...
  LUTHIER_RETURN_ON_ERROR(
      getAllAgentsWithDeviceType<HSA_DEVICE_TYPE_GPU>(CoreApi, Agents));
  ...
  for (const hsa_agent_t Agent : Agents) {
    ...
    std::optional<uint32_t> Found = kfd::gpuIdForTopologyNode(Node);
    if (Found && *Found == GpuId) {
      LLVM_DEBUG(luthier::dbgs() << llvm::formatv(
                     "[Agent] gpu_id {0} is topology node {1}, agent {2:x}\n",
