...
//===----------------------------------------------------------------------===//
/// \file TraceFunctionTranslationAnalysis.cpp
/// Implements the \c TraceFunctionTranslationAnalysis and its \c
...
#define DEBUG_TYPE "luthier-trace-function-translation"
...
void TranslationState::markDirty(const llvm::MachineBasicBlock &MBB) {
  ...
  if (!MD) {
    ...
    if (!MDOrErr) {
      LLVM_DEBUG(
          luthier::dbgs()
              << "[TraceFunctionTranslation] Failed to persist dirty mark: "
...
llvm::Error TranslationState::flushFull() {
  ...
  Translator = std::make_unique<TraceFunctionTranslator>(MF, Err);
  if (Err) {
    Translator.reset();
    return Err;
  }
...
llvm::Error TranslationState::flush() {
  ...
  LLVM_DEBUG(luthier::dbgs()
                 << "[TraceFunctionTranslation] Flushing " << DirtyMBBs.size()
  ...
  LLVM_DEBUG(luthier::dbgs()
                 << "[TraceFunctionTranslation] Incremental flush of "
...
llvm::AnalysisKey TraceFunctionTranslationAnalysis::Key;

TraceFunctionTranslationAnalysis::Result
TraceFunctionTranslationAnalysis::run(llvm::MachineFunction &MF,
