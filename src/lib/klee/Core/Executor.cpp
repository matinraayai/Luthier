...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    : Interpreter(opts), IHandler(ih), Searcher(nullptr),
      ExtDispatcher(new ExternalDispatcher(ctx)), statsTracker(0),
  ...
  std::unique_ptr<Solver> solver = constructSolverChain(
      std::move(coreSolver),
      IHandler.getOutputFilename(ALL_QUERIES_SMT2_FILE_NAME),
      IHandler.getOutputFilename(SOLVER_QUERIES_SMT2_FILE_NAME),
      IHandler.getOutputFilename(ALL_QUERIES_KQUERY_FILE_NAME),
      IHandler.getOutputFilename(SOLVER_QUERIES_KQUERY_FILE_NAME));
  ...
  if (DebugPrintInstructions.isSet(FILE_ALL) ||
    ...
    std::string debug_file_name =
        IHandler.getOutputFilename("instructions.txt");
    std::string error;
#ifdef HAVE_ZLIB_H
...
Executor::setModule(std::vector<std::unique_ptr<llvm::Module>> &modules,
  ...
  kmodule->manifest(IHandler, StatsTracker::useStatistics());

  specialFunctionHandler->bind();

  if (StatsTracker::useStatistics() || userSearcherRequiresMD2U()) {
    statsTracker =
        new StatsTracker(*this, IHandler.getOutputFilename("assembly.ll"),
...
void Executor::doDumpStates() {
  if (!DumpStatesOnHalt || states.empty()) {
    IHandler.incPathsExplored(states.size());
    return;
  }
...
void Executor::terminateStateEarly(ExecutionState &state, const Twine &message,
  ...
  if ((reason <= StateTerminationType::EARLY && shouldWriteTest(state)) ||
    ...
    IHandler.processTestCase(state, (message + "\n").str().c_str(),
...
void Executor::terminateStateOnError(ExecutionState &state,
  ...
  if (EmitAllErrors ||
    ...
    IHandler.processTestCase(state, msg.str().c_str(), file_suffix);
  }
...
void Executor::callExternalFunction(ExecutionState &state, KInstruction *target,
  ...
  uint64_t *args = static_cast<uint64_t *>(alloca(allocatedBytes));
  ...
  bool success = ExtDispatcher->executeCall(callable, target->inst, args);
  if (!success) {
    terminateStateOnExecError(state,
...
void Executor::resolveExact(ExecutionState &state, ref<Expr> p,
  ...
  for (ResolutionList::iterator it = rl.begin(), ie = rl.end(); it != ie;
    ...
    if (branches.first)
      results.emplace_back(*it, branches.first);
...
void Executor::runFunctionAsMain(Function *f, int argc, char **argv,
  ...
  executionTree = createExecutionTree(
      *state, userSearcherRequiresInMemoryExecutionTree(), *IHandler);
  run(*state);
...
void Executor::dumpExecutionTree() {
  ...
  auto os = IHandler.openOutputFile(name);
  if (os)
...
void Executor::dumpStates() {
  ...
  auto os = IHandler.openOutputFile("states.txt");
