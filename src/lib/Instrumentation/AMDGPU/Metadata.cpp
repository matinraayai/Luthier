...
AMDGCNMetadataParserAnalysisPass::Result
AMDGCNMetadataParserAnalysisPass::run(llvm::Module &TargetModule,
                                      llvm::ModuleAnalysisManager &TargetMAM) {
  llvm::LLVMContext &Ctx = TargetModule.getContext();
  /// Get the object file being lifted and obtain its metadata document
  const llvm::object::ObjectFile &ObjFile =
      TargetMAM.getResult<ObjectFileAnalysisPass>(TargetModule).getObject();
  if (!llvm::isa<object::AMDGCNObjectFile>(ObjFile))
    Ctx.emitError("Object file is not AMDGCN");
  const auto &AMDGCNObjFile = llvm::cast<object::AMDGCNObjectFile>(ObjFile);
  llvm::Expected<std::unique_ptr<Document>> DocOrErr =
      AMDGCNObjFile.getMetadataDocument();
  if (llvm::Error Err = DocOrErr.takeError())
    Ctx.emitError(llvm::toString(std::move(Err)));

  /// Parse the object's metadata
  llvm::Expected<std::unique_ptr<amdgpu::hsamd::Metadata>> ObjMetadataOrErr =
      Parser.parseNoteMetaData(**DocOrErr);
  if (llvm::Error Err = ObjMetadataOrErr.takeError())
    Ctx.emitError(llvm::toString(std::move(Err)));

  /// Parse the kernel's metadata
  object::AMDGCNKernelDescSymbolRef KernelSym =
      TargetMAM.getResult<AMDGPULiftedKernelSymbolAnalysisPass>(TargetModule)
          .getSymbol();

  llvm::StringRef KernelName;
  if (llvm::Error Err = KernelSym.getName().moveInto(KernelName)) {
    Ctx.emitError(llvm::toString(std::move(Err)));
  }

  llvm::Expected<std::unique_ptr<amdgpu::hsamd::Kernel::Metadata>>
      KernelMetadataOrErr = Parser.parseKernelMetadata(**DocOrErr, KernelName);
  if (llvm::Error Err = KernelMetadataOrErr.takeError()) {
    Ctx.emitError(llvm::toString(std::move(Err)));
  }

  return Result{std::move(*ObjMetadataOrErr), std::move(*KernelMetadataOrErr)};
}
} // namespace luthier
