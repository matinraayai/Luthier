...
disassemble(hsa_executable_symbol_t Func, bool includeDebugInfo = false) {
...
lift(hsa_executable_symbol_t Kernel, bool includeDebugInfo = false) {
  auto KernelWrapper = hsa::ExecutableSymbol::fromHandle(Kernel);
  LUTHIER_RETURN_ON_ERROR(KernelWrapper.takeError());
  return CodeLifter::instance().lift(*KernelWrapper, includeDebugInfo);
}

llvm::Expected<const luthier::LiftedRepresentation &>
lift(hsa_executable_t Executable, bool includeDebugInfo = false) {
  return CodeLifter::instance().lift(hsa::Executable{Executable}, includeDebugInfo);
}
