  ...
  auto &ModuleEntry = LR.Modules
                          .insert({LCO.asHsaType(),
                                   std::move(std::make_pair(std::move(TSModule),
                                                            std::move(MMIWP)))})
                          .first->getSecond();
  LR.RelatedLCOs.insert({LCO.asHsaType(), {&ModuleEntry.first, &MMI}});
  return llvm::Error::success();
}
...
CodeLifter::initLiftedGlobalVariableEntry(const hsa::LoadedCodeObject &LCO,
  ...
  auto &Module = *LR.RelatedLCOs.at(LCO.asHsaType()).first->getModuleUnlocked();
...
llvm::Error CodeLifter::liftFunction(
    ...
    llvm::DenseMap<hsa::ExecutableSymbol, bool> &SymbolUsageMap, bool includeDebugInfo = false) {
  auto LCO = Symbol.getDefiningLoadedCodeObject();
  LUTHIER_RETURN_ON_ERROR(LUTHIER_ASSERTION(LCO.has_value()));

  llvm::Module &Module =
      *LR.RelatedLCOs.at(LCO->asHsaType()).first->getModuleUnlocked();
  llvm::MachineModuleInfo &MMI = *LR.RelatedLCOs.at(LCO->asHsaType()).second;
...
luthier::CodeLifter::lift(const hsa::ExecutableSymbol &Symbol, bool includeDebugInfo = false) {
  if (!LiftedKernelSymbols.contains(Symbol)) {
    ...
    for (const auto &LCO : UsedLCOs) {
      LR.Modules.erase(LCO.asHsaType());
...
llvm::Expected<const LiftedRepresentation &>
CodeLifter::lift(const hsa::Executable &Exec, bool includeDebugInfo = false) {
