...
template <>
luthier::hip::CompilerInterceptor
    *luthier::Singleton<luthier::hip::CompilerInterceptor>::Instance{nullptr};

static hipError_t __hipPopCallConfiguration_callback(dim3 *gridDim,
                                                     dim3 *blockDim,
                                                     size_t *sharedMem,
                                                     hipStream_t *stream) {
  ...
  if (ShouldCallback) {
    ...
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_BEFORE, ApiId,
                          &SkipFunction);
    if (!SkipFunction)
      Out = HipInterceptor.getSavedCompilerTable().__hipPopCallConfiguration_fn(
          Args.__hipPopCallConfiguration.gridDim,
          Args.__hipPopCallConfiguration.blockDim,
          Args.__hipPopCallConfiguration.sharedMem,
          Args.__hipPopCallConfiguration.stream);
    if (IsUserCallbackEnabled)
    ...
    return HipInterceptor.getSavedCompilerTable().__hipPopCallConfiguration_fn(
        gridDim, blockDim, sharedMem, stream);
  }
}

static hipError_t __hipPushCallConfiguration_callback(dim3 gridDim,
                                                      dim3 blockDim,
                                                      size_t sharedMem,
                                                      hipStream_t stream) {
  auto &HipInterceptor = luthier::hip::CompilerInterceptor::instance();
  ...
  if (ShouldCallback) {
    ...
    rocprofiler_hip_api_args_t Args;
    bool SkipFunction{false};
    Args.__hipPushCallConfiguration.gridDim =
        luthier::convertToRocprofilerDim3(gridDim);
    Args.__hipPushCallConfiguration.blockDim =
        luthier::convertToRocprofilerDim3(blockDim);
    ...
    if (!SkipFunction)
      Out =
          HipInterceptor.getSavedCompilerTable().__hipPushCallConfiguration_fn(
              gridDim, blockDim, Args.__hipPushCallConfiguration.sharedMem,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipPushCallConfiguration_fn(
        gridDim, blockDim, sharedMem, stream);
  }
}
  ...
  if (ShouldCallback) {
    ...
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_BEFORE, ApiId,
                          &SkipFunction);
    if (!SkipFunction)
      Out = HipInterceptor.getSavedCompilerTable().__hipRegisterFatBinary_fn(
          Args.__hipRegisterFatBinary.data);
    if (IsUserCallbackEnabled)
      HipUserCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId);
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipRegisterFatBinary_fn(
        data);
  }
}
  ...
  if (ShouldCallback) {
    ...
    if (!SkipFunction)
      HipInterceptor.getSavedCompilerTable().__hipRegisterFunction_fn(
          ...
          Args.__hipRegisterFunction.tid, Args.__hipRegisterFunction.bid,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipRegisterFunction_fn(
        modules, hostFunction, deviceFunction, deviceName, threadLimit, tid,
        bid, blockDim, gridDim, wSize);
  }
}

static void __hipRegisterManagedVar_callback(void *hipModule, void **pointer,
  ...
  if (ShouldCallback) {
    ...
    if (!SkipFunction)
      HipInterceptor.getSavedCompilerTable().__hipRegisterManagedVar_fn(
          Args.__hipRegisterManagedVar.hipModule,
          Args.__hipRegisterManagedVar.pointer,
          Args.__hipRegisterManagedVar.init_value,
          Args.__hipRegisterManagedVar.name, Args.__hipRegisterManagedVar.size,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipRegisterManagedVar_fn(
        hipModule, pointer, init_value, name, size, align);
  }
}

static void __hipRegisterSurface_callback(void **modules, void *var,
  ...
  if (ShouldCallback) {
    ...
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_BEFORE, ApiId,
                          &SkipFunction);
    if (!SkipFunction)
      HipInterceptor.getSavedCompilerTable().__hipRegisterSurface_fn(
          Args.__hipRegisterSurface.modules, Args.__hipRegisterSurface.var,
          Args.__hipRegisterSurface.hostVar,
          Args.__hipRegisterSurface.deviceVar, Args.__hipRegisterSurface.type,
          Args.__hipRegisterSurface.ext);
    if (IsUserCallbackEnabled)
      HipUserCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId);
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipRegisterSurface_fn(
        modules, var, hostVar, deviceVar, type, ext);
  }
}

static void __hipRegisterTexture_callback(void **modules, void *var,
                                          char *hostVar, char *deviceVar,
  ...
  if (ShouldCallback) {
    ...
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_BEFORE, ApiId,
                          &SkipFunction);
    if (!SkipFunction)
      HipInterceptor.getSavedCompilerTable().__hipRegisterTexture_fn(
          Args.__hipRegisterTexture.modules, Args.__hipRegisterTexture.var,
          Args.__hipRegisterTexture.hostVar,
          Args.__hipRegisterTexture.deviceVar, Args.__hipRegisterTexture.type,
          Args.__hipRegisterTexture.norm, Args.__hipRegisterTexture.ext);
    if (IsUserCallbackEnabled)
      HipUserCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId);
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipRegisterTexture_fn(
        modules, var, hostVar, deviceVar, type, norm, ext);
  }
}

static void __hipRegisterVar_callback(void **modules, void *var, char *hostVar,
  ...
  if (ShouldCallback) {
    ...
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_BEFORE, ApiId,
                          &SkipFunction);
    if (!SkipFunction)
      HipInterceptor.getSavedCompilerTable().__hipRegisterVar_fn(
          Args.__hipRegisterVar.modules, Args.__hipRegisterVar.var,
          Args.__hipRegisterVar.hostVar, Args.__hipRegisterVar.deviceVar,
          Args.__hipRegisterVar.ext, Args.__hipRegisterVar.size,
          Args.__hipRegisterVar.constant, Args.__hipRegisterVar.global);
    if (IsUserCallbackEnabled)
      HipUserCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId);
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipRegisterVar_fn(
  ...
  if (ShouldCallback) {
    ...
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_BEFORE, ApiId,
                          &SkipFunction);
    if (!SkipFunction)
      HipInterceptor.getSavedCompilerTable().__hipUnregisterFatBinary_fn(
          Args.__hipUnregisterFatBinary.modules);
    if (IsUserCallbackEnabled)
      HipUserCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId);
    if (IsInternalCallbackEnabled)
      HipInternalCallback(&Args, luthier::API_EVT_PHASE_AFTER, ApiId,
    ...
    return HipInterceptor.getSavedCompilerTable().__hipUnregisterFatBinary_fn(
        modules);
  }
}

void luthier::hip::CompilerInterceptor::captureCompilerDispatchTable(
    HipCompilerDispatchTable *CompilerTable) {
  SavedCompilerDispatchTable = *CompilerTable;
  CompilerTable->__hipPopCallConfiguration_fn =
      __hipPopCallConfiguration_callback;
  CompilerTable->__hipPushCallConfiguration_fn =
      __hipPushCallConfiguration_callback;
  ...
  CompilerTable->__hipUnregisterFatBinary_fn =
      __hipUnregisterFatBinary_callback;
};
