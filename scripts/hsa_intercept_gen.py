...
def main():
    ...
    callback_defs.append("void QueueSubmitWriteInterceptor(const void *Packets, uint64_t PktCount,"
                         ...
                         "\tstd::vector<luthier_hsa_aql_packet_t> modifiedPackets(reinterpret_cast<const luthier::HsaAqlPacket*>(Packets), "
        ...
        for f in api_table.fields:
            ...
            if isinstance(f.type, cxx_types.Pointer) and isinstance(f.type.ptr_to.typename.segments[0],
                ...
                if hsa_function_name == "hsa_queue_create": # Create intercept queues instead
                    callback_defs.append("\t\t\tOut = HsaInterceptor.getSavedHsaTables().amd_ext.hsa_amd_queue_intercept_create_fn("
                                         ...
                                         "\t\t\t\treturn Out;\n"
                                         "\t\t\tOut = HsaInterceptor.getSavedHsaTables().amd_ext.hsa_amd_queue_intercept_register_fn("
                                         "*Args.hsa_queue_create.queue, QueueSubmitWriteInterceptor, *Args.hsa_queue_create.queue);\n"
                                         "\t\t\tif (Out != HSA_STATUS_SUCCESS)")
                else:
                    if return_type != "void":
