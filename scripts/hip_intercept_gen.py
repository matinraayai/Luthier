...
def generate_api_id_dense_map_info(api_names: List[str]):
    api_id_dense_map_info = ["namespace llvm {\n"]
    for api_name in api_names:
        api_id_dense_map_info.append(f"""

template <> struct DenseMapInfo<luthier::hip::{api_name}ApiEvtID> {{
  static inline luthier::hip::{api_name}ApiEvtID getEmptyKey() {{                                        
    return (luthier::hip::{api_name}ApiEvtID)(DenseMapInfo<std::underlying_type_t<luthier::hip::{api_name}ApiEvtID>>::getEmptyKey());
  }}
  
  static inline luthier::hip::{api_name}ApiEvtID getTombstoneKey() {{
  return (luthier::hip::{api_name}ApiEvtID)(DenseMapInfo<std::underlying_type_t<luthier::hip::{api_name}ApiEvtID>>::getTombstoneKey());
  }}

  static unsigned getHashValue(const luthier::hip::{api_name}ApiEvtID &P) {{
  return DenseMapInfo<std::underlying_type_t<luthier::hip::{api_name}ApiEvtID>>::getHashValue(
    static_cast<std::underlying_type<luthier::hip::{api_name}ApiEvtID>::type>(P));
  }}
  
  static bool isEqual(const luthier::hip::{api_name}ApiEvtID &LHS, const luthier::hip::{api_name}ApiEvtID &RHS) {{
    return static_cast<std::underlying_type<luthier::hip::{api_name}ApiEvtID>::type>(LHS) ==
           static_cast<std::underlying_type<luthier::hip::{api_name}ApiEvtID>::type>(RHS);
  }}
  
}};
""")
    api_id_dense_map_info.append("""
};
...
def main():
    ...
    dense_map_info = generate_api_id_dense_map_info(api_names)
