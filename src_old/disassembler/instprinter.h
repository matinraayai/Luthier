...
struct InstPrinter {
  ...
  std::string sop2String(const Inst& i) {
  ...
  std::string sopkString(const Inst& i) {
    std::stringstream stream;
    stream << i.instType.instName << " " << operandString(i.dst) << ", "
  ...
  std::string sop1String(const Inst& i) {
    std::stringstream stream;
  ...
  std::string sopcString(const Inst& i) {
    std::stringstream stream;
    stream << i.instType.instName << " " << operandString(i.src0) << ", "
  ...
  std::string soppString(const Inst& i) {
    std::stringstream stream;
    ...
    } else if (i.instType.opcode >= 2 && i.instType.opcode <= 9) // Branch
    {
      ...
      int16_t imm = int16_t(uint16_t(i.simm16.intValue_));
  ...
  std::string sdwaSelectString(int sdwaSel) {
	...
	std::string sdwaString(const Inst& i){
		std::stringstream stream;
  ...
  std::string vop1String(const Inst &i) {
    std::stringstream stream;
    std::string suffix;
  ...
  std::string vop2String(const Inst& i) {
    std::stringstream stream;
    std::string suffix;
		i.IsSdwa ? suffix = "_sdwa" : suffix = "_e32";
    stream << i.instType.instName << suffix << " " << operandString(i.dst);
  ...
  std::string vopcString(const Inst& i) {
    std::stringstream stream;
    std::string dst = "vcc";
    if (i.instType.instName.find("cmpx") != std::string::npos) {
      dst = "exec";
    }
    stream << i.instType.instName << "_e32 " << dst << ", "
           << operandString(i.src0) << ", " << operandString(i.src1);
    return stream.str();
  }

  std::string vop3aString(const Inst& i) {
    std::stringstream stream;
    std::string suffix = "";
    ...
    stream << i.instType.instName + suffix << " " << operandString(i.dst);
    stream << ", "
           << vop3aInputoperandString(i, i.src0, i.Src0Neg, i.Src0Abs);
    stream << ", "
    ...
    stream << ", "
           << vop3aInputoperandString(i, i.src2, i.Src2Neg, i.Src2Abs);
    return stream.str();
  }

  std::string vop3aInputoperandString(const Inst& i, Operand o, bool neg, bool abs) {
    std::stringstream s;
    if (neg) {
      s << "-";
    }
  ...
  std::string vop3bString(const Inst& i) {
    std::stringstream stream;
    stream << i.instType.instName << " ";
    ...
    stream << operandString(i.sdst) << ", " << operandString(i.src0) << ", "
           << operandString(i.src1);

    if (i.instType.opcode > 255 && i.instType.opcode != 281 &&
        i.instType.SRC2Width > 0) {
      stream << ", " << operandString(i.src2);
    }
    return stream.str();
  }

	std::string vop3pString(const Inst& i) {
		std::stringstream stream;
  ...
  std::string flatString(const Inst& i) {
    std::stringstream stream;
    ...
    if (opcode >= 16 && opcode <= 23) {
      if (i.sAddr.operandType_ != RegOperand) {
        stream << i.instType.instName << " " << operandString(i.dst) << ", "
    ...
    if (i.Seg == 2) {
      if (opcode >= 16 && opcode <= 18 || opcode == 20 || opcode == 21 ||
        ...
        if (i.sAddr.operandType_ != RegOperand) {
          stream << ", off";
          if (i.offset.intValue_ != 0) {
            stream << " offset:" << (int)i.offset.intValue_;
          }
        } 
  ...
  std::string dsString(const Inst& i) {
    std::stringstream stream;
    auto oc = i.instType.opcode;
    stream << i.instType.instName << " ";
    if (oc >= 54 && oc <= 60 || oc >= 118 && oc <= 120 ||
        oc >= 254 && oc <= 255) {
      stream << operandString(i.dst) << ", ";
    }
