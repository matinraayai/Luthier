...
Disassembler::Disassembler(elfio::File *file): nextInstID(0) {
  initFormatList();
  initRegs();
  initializeDecodeTable();
  printer = InstPrinter(file);
}
Disassembler::Disassembler() : nextInstID(0) {
  initFormatList();
  initRegs();
  initializeDecodeTable();
  printer = InstPrinter();
...
void Disassembler::Disassemble(elfio::File *file, std::string filename,
  ...
  while (!buf.empty()) {
    ...
    Inst inst = decode(buf.data());
    inst.PC = pc;
    std::string instStr = printer.print(inst);
    o << "\t" << instStr;
    ...
    o << std::setbase(10) << "//" << std::setw(12) << std::setfill('0') << pc
      << ": ";
    o << std::setw(8) << std::hex << convertLE(buf.data());
    if (inst.byteSize == 8) {
      std::vector<unsigned char> sfb(buf.begin() + 4, buf.begin() + 8);
      o << std::setw(8) << std::hex << convertLE(sfb.data()) << std::endl;
    } else {
      o << std::endl;
    }
...
std::vector<Inst> Disassembler::Disassemble(const unsigned char *buf) {
  uint64_t pc = 0;
  std::vector<Inst> out;
  while (out.empty() || (out.end() - 1)->instType.instName != "s_endpgm") {
    Inst inst = decode(buf + pc);
    inst.PC = pc;
    pc += uint64_t(inst.byteSize);
...
void Disassembler::Disassemble(std::vector<unsigned char> buf,
  ...
  while (!buf.empty()) {
    ...
    currInst->instStr = printer.print(inst);
...
InstType Disassembler::lookUp(const Format& format, Opcode opcode) {
...
Inst Disassembler::decode(const unsigned char* buf) {
...
//  }
  ...
  case VOPC:
    ...
    break;
  case VOP3a:
    decodeVOP3a(inst, buf);
    break;
  case VOP3b:
    decodeVOP3b(inst, buf);
    break;
  case VOP3P:
    decodeVOP3p(inst, buf);
    break;
  case FLAT:
    decodeFLAT(inst, buf);
    break;
...
//    std::vector<unsigned char> sub(&buf[4], &buf[8]);
  ...
  if (inst.instType.instName.find("64") != std::string::npos) {
    inst.src0.regCount_ = 2;
    inst.src1.regCount_ = 2;
    inst.dst.regCount_ = 2;
  }
}

void Disassembler::decodeSOPK(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
...
void Disassembler::decodeSOP1(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
...
void Disassembler::decodeSOPC(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
...
void Disassembler::decodeSOPP(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
...
void Disassembler::decodeSMEM(Inst& inst, const unsigned char* buf) {
  auto bytesLo = convertLE(buf);
//  std::vector<unsigned char> sfb(buf.begin() + 4, buf.end());
  auto bytesHi = convertLE(buf + 4);
...
void Disassembler::decodeVOP1(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
...
void Disassembler::decodeVOP2(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
  if (extractBitsFromU32(bytes, 0, 8) == 249) {
    decodeSDWA(inst, buf);
  } else {
...
//    std::vector<unsigned char> sub(&buf[4], &buf[8]);
  ...
  if (inst.instType.opcode == 24 || inst.instType.opcode == 37) { // madak
    ...
    inst.byteSize += 4;
    Operand o;
    o.operandType_ = LiteralConstant;
    inst.src2 = o;
//    std::vector<unsigned char> sub(&buf[4], &buf[8]);
    inst.src2.literalConstant_ = convertLE(buf + 4);
  }
}

void Disassembler::decodeVOPC(Inst& inst, const unsigned char* buf) {
  uint32_t bytes = convertLE(buf);
  uint32_t src0Value = extractBitsFromU32(bytes, 0, 8);
  inst.src0 = getOperandByCode(uint16_t(src0Value));
  if (inst.src0.operandType_ == LiteralConstant) {
...
void Disassembler::decodeVOP3a(Inst& inst, const unsigned char* buf) {
  auto bytesLo = convertLE(buf);
  auto bytesHi = convertLE(buf + 4);
...
void Disassembler::decodeSDWA(Inst& inst, const unsigned char* buf) {
//  if (buf.size() < 8)
...
//    sdwabuf.push_back(buf.at(i));
  uint32_t sdwaBytes = convertLE(buf + 4);
...
void Disassembler::decodeVOP3b(Inst& inst, const unsigned char* buf) {
  auto bytesLo = convertLE(buf);
  auto sfb = buf + 4;
...
void Disassembler::decodeVOP3p(Inst& inst, const unsigned char* buf) {
  auto bytesLo = convertLE(buf);
  auto bytesHi = convertLE(buf + 4);
...
void Disassembler::decodeFLAT(Inst& inst, const unsigned char* buf) {
  auto bytesLo = convertLE(buf);
  auto bytesHi = convertLE(buf + 4);
  ...
  case 98:
    ...
    break;
  case 28:
    if (inst.sAddr.code_ != 0x7F) {
      inst.addr.regCount_ = 1;
    }
    inst.dst.regCount_ = 4;
    break;
  case 30:
    inst.sAddr.regCount_ = 4;
    inst.data.regCount_ = 4;
    inst.dst.regCount_ = 4;
    break;
  }
...
void Disassembler::decodeDS(Inst& inst, const unsigned char* buf) {
  auto bytesLo = convertLE(buf);
  auto bytesHi = convertLE(buf + 4);
