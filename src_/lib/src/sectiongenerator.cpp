...
void getSymtabSecBinary(char *newBinary, elfio::Section *pSec,
  ...
  // modify section idx new .bss is at 13 for "counter", "counter.managed"
  symTable[3].st_shndx = 13;
  symTable[3].st_name = 86;
  symTable[6].st_shndx = 13;
  symTable[6].st_name = 94;
  ...
  tSym->st_name = 110; // MISSING symbol name in strtab
...
void getStrtabSecBinary(char *newBinary, elfio::Section *pSec,
                        elfio::Section *iSec) {
  std::memcpy(newBinary, pSec->Blob(), pSec->size);
  int offset = pSec->size;                             // offset=73
  ...
  offset += str1.size() + 1; // 86
  std::memcpy(newBinary + offset, iSec->Blob() + 14, str2.size() + 1);
  offset += str2.size() + 1; // 94
  std::memcpy(newBinary + offset, iSec->Blob() + 0x27, str3.size() + 1);
  offset += str3.size() + 1; // 110
  std::memcpy(newBinary + offset, str4.c_str(),
              str4.size() + 1); // new size 121
}
